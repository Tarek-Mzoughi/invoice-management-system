...
CREATE TABLE
    IF NOT EXISTS `payment-upload` (
        ...
        CONSTRAINT `FK_upload_payment-upload` FOREIGN KEY (`uploadId`) REFERENCES `storage` (`id`) ON DELETE CASCADE
    );
...
SET @invoice_has_customer_order_id = (
    SELECT COUNT(*)
    FROM `information_schema`.`columns`
    WHERE `table_schema` = DATABASE()
      AND `table_name` = 'invoice'
      AND `column_name` = 'customerOrderId'
);

SET @invoice_cleanup_customer_order_sql = IF(
    @invoice_has_customer_order_id > 0,
    '
    UPDATE `invoice` i
...
PREPARE `invoice_cleanup_customer_order_stmt` FROM @invoice_cleanup_customer_order_sql;
EXECUTE `invoice_cleanup_customer_order_stmt`;
DEALLOCATE PREPARE `invoice_cleanup_customer_order_stmt`;

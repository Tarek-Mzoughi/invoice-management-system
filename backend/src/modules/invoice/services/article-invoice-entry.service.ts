...
export class ArticleInvoiceEntryService {
  ...
  async save(
    ...
    const article =
      (await this.articleService.findOneByCondition({
        filter: `title||$eq||${createArticleInvoiceEntryDto.article.title};cabinetId||$eq||${normalizedCabinetId}`,
  ...
  async update(
    ...
    const article =
      (await this.articleService.findOneByCondition({
        filter: `title||$eq||${updateArticleInvoiceEntryDto.article.title};cabinetId||$eq||${normalizedCabinetId}`,
      })) || (await this.articleService.save(articleToCreate));

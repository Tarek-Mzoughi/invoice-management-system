...
export function ClientPortal({ className }: ClientPortalProps) {
  ...
  const list = useFirmList(CLIENT_FIRM_MODULE_CONFIG.entityType);
  const prefix = 'firm.modules.clients';
  ...
  React.useEffect(() => {
    ...
    setFloating?.(
      <Button
        ...
        onClick={() => guardedNavigation.push(CLIENT_FIRM_MODULE_CONFIG.newPath)}
      >

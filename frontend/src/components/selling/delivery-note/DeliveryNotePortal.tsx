...
import { ArrowUpRight, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, Copy, Download, Edit, ExternalLink, FileText, Eye, Mail, MessageCircle, MoreHorizontal, Paperclip, PencilLine, Plus, RefreshCw, Search, Settings2, Trash2, User2, CheckCircle2, X } from 'lucide-react';
import { api } from '@/api';
...
export const DeliveryNotePortal: React.FC<DeliveryNoteMainProps> = ({ className, scope = 'selling', rootPath = '/selling', listPath = '/selling/delivery-notes', newPath = '/selling/new-delivery-note', detailPathPrefix = '/selling/delivery-note', linkedInvoicePathPrefix = '/selling/invoice', linkedReturnNotePathPrefix = '/selling/return-note' }) => {
    ...
    React.useEffect(() => {
        ...
        {selectedIds.length > 0 && (<DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-64">
              <DropdownMenuItem onClick={handleBulkDownload} disabled={selectedIds.length === 0} className="cursor-pointer">
              ...
              <DropdownMenuItem onClick={() => bulkTransformToReturnNote(selectedIds)} disabled={selectedIds.length === 0} className="cursor-pointer">
                <FileText className="mr-2 h-4 w-4"/>
                {`${tCommon('commands.transform_to')} ${returnNoteLabels.singular}`}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => setBulkDeleteDialog(true)} disabled={selectedIds.length === 0} className="cursor-pointer text-rose-600 focus:text-rose-600 focus:bg-rose-50">
        ...
        <Button className="h-11 rounded-md bg-amber-500 px-5 font-medium text-zinc-950 shadow-sm hover:bg-amber-400" onClick={() => router.push(newPath)}>
    ...
    const setManagedDeliveryNote = React.useCallback((deliveryNote: DeliveryNote) => {
        if (!deliveryNote.id)
            return false;
    ...
    const closeDeliveryNoteDetailsThen = React.useCallback((callback: () => void) => runAfterOverlayClose(() => setDetailsDialog(false), callback), [runAfterOverlayClose]);
    ...
    const { mutate: loadDeliveryNotePreview, isPending: isDeliveryNotePreviewPending } = useMutation({
        ...
        onError: (error) => {
            toast.error(getErrorMessage('invoicing', error, tInvoicing('invoice.action_preview_failure')));
            closePreviewDialog();
        }
    });
    const { mutate: loadInvoicePreview, isPending: isInvoicePreviewPending } = useMutation({
        ...
        onError: (error) => {
            toast.error(getErrorMessage('invoicing', error, tInvoicing('deliveryNote.action_preview_failure')));
            closePreviewDialog();
        }
    });
    const { mutate: loadReturnNotePreview, isPending: isReturnNotePreviewPending } = useMutation({
        ...
        onError: (error) => {
            toast.error(getErrorMessage('invoicing', error, tInvoicing('returnNote.action_preview_failure')));
            closePreviewDialog();
        }
    });
    const openDeliveryNotePreview = React.useCallback((deliveryNote: DeliveryNote) => {
        if (!deliveryNote.id)
            return;
        setSelectedDeliveryNote(deliveryNote);
        openPreviewDialog(`${deliveryNoteLabels.displayNumber(deliveryNote) || `delivery-note-${deliveryNote.id}`}.pdf`);
    ...
    const openLinkedDocumentPreview = React.useCallback((type: string, id: number) => {
        ...
        if (type === 'return-note') {
            const returnNote = selectedDeliveryNote?.returnNotes?.find((document) => document.id === id);
    ...
    const handleDownloadWithAttachments = React.useCallback(async (deliveryNote: DeliveryNote) => {
        try {
            if (!deliveryNote.id)
                return;
            ...
            toast.error(getErrorMessage('invoicing', error as Error, tInvoicing('deliveryNote.action_download_failure')));
        }
    ...
    const { mutate: updateStatus, isPending: isUpdateStatusPending } = useMutation({
        ...
        onError: (error) => {
            toast.error(getErrorMessage('invoicing', error, tInvoicing('deliveryNote.action_update_failure')));
        }
    });
    // --- Bulk Actions ---
    const { mutate: bulkTransformToInvoice, isPending: isBulkInvoicePending } = useMutation({
        ...
        onError: (error) => {
            toast.error(getErrorMessage('invoicing', error, tInvoicing('deliveryNote.action_invoice_failure')));
        }
    });
    ...
    const isPreviewPending = isDeliveryNotePreviewPending || isInvoicePreviewPending || isReturnNotePreviewPending;
    ...
    return (<>
      <DeliveryNoteBulkDeleteDialog count={selectedIds.length} open={bulkDeleteDialog} onDelete={handleBulkDelete} isPending={isBulkDeleting} onClose={() => setBulkDeleteDialog(false)}/>
      <DeliveryNoteDeleteDialog id={deliveryNoteManager?.id} sequential={deliveryNoteManager?.sequential || ''} open={deleteDialog} deleteDeliveryNote={() => {
      ...
      <DeliveryNoteDuplicateDialog id={deliveryNoteManager?.id || 0} sequential={deliveryNoteManager?.sequential || ''} open={duplicateDialog} duplicateDeliveryNote={(includeFiles: boolean) => {
      ...
      <DeliveryNoteDownloadDialog id={deliveryNoteManager?.id || 0} open={downloadDialog} downloadDeliveryNote={(template: string) => {
            deliveryNoteManager?.id &&
      ...
      <DeliveryNoteInvoiceDialog id={deliveryNoteManager?.id || 0} status={deliveryNoteManager?.status} sequential={deliveryNoteManager?.sequential} open={invoiceDialog} isInvoicePending={isInvoicingPending} invoice={(id: number, createInvoice: boolean) => {
            invoiceDeliveryNote({ id, createInvoice });
        }} scope={scope} onClose={() => setInvoiceDialog(false)}/>
      <DeliveryNoteActionDialog id={deliveryNoteManager?.id} action={`${tCommon('commands.transform_to')} ${returnNoteLabels.singular}`} sequential={deliveryNoteManager?.sequential || ''} open={returnNoteDialog} callback={() => deliveryNoteManager?.id && toReturnNote(deliveryNoteManager.id)} isCallbackPending={isReturnNotePending} scope={scope} onClose={() => setReturnNoteDialog(false)}/>
      <DeliveryNoteStatusDialog open={statusDialog} deliveryNote={selectedDeliveryNote} isPending={isUpdateStatusPending} scope={scope} callback={(status) => {
            if (selectedDeliveryNote?.id) {
                updateStatus({ id: selectedDeliveryNote.id, status });
            }
        }} onClose={() => setStatusDialog(false)}/>
      <DocumentPreviewDialog open={previewDialog} loading={isPreviewPending} previewBlob={previewBlob} filename={previewFilename} title={tCommon('commands.preview')} onClose={closePreviewDialog}/>
      <DeliveryNoteDetailsDialog open={detailsDialog} onClose={() => setDetailsDialog(false)} deliveryNote={selectedDeliveryNote} detailPathPrefix={detailPathPrefix} scope={scope} onPreview={() => {
            if (!selectedDeliveryNote)
                return;
            closeDeliveryNoteDetailsThen(() => openDeliveryNotePreview(selectedDeliveryNote));
        }} onDownload={() => {
            if (!selectedDeliveryNote || !setManagedDeliveryNote(selectedDeliveryNote))
                return;
      ...
      <DeliveryNoteEmailDialog open={emailDialog} onClose={() => setEmailDialog(false)} deliveryNote={selectedDeliveryNote} scope={scope}/>
      <DeliveryNoteWhatsAppDialog open={whatsAppDialog} onClose={() => setWhatsAppDialog(false)} deliveryNote={selectedDeliveryNote} scope={scope}/>
      <DeliveryNoteAttachmentDialog open={attachmentDialog} onClose={() => setAttachmentDialog(false)} deliveryNote={selectedDeliveryNote} scope={scope}/>
            ...
            <div className="space-y-2">
              ...
              <Select value={filters.status} onValueChange={(value) => handleFilterChange('status', value)}>
                ...
                <SelectContent>
                  ...
                  {DELIVERY_NOTE_STATUS_OPTIONS.map((status) => (<SelectItem key={status} value={status}>
                      {tInvoicing(status)}
                    </SelectItem>))}
                </SelectContent>
              </Select>
        ...
        <section className={cn(panelClassName, 'flex min-h-0 flex-1 flex-col')}>
          <div className="min-h-0 flex-1 overflow-auto">
            ...
            setSelectedIds([]);
                  ...
                  <th className="px-4 py-3.5 font-medium">
                    {tInvoicing('deliveryNote.attributes.number')}
                  </th>
                  <th className="px-4 py-3.5 font-medium">
              ...
              <tbody>
                {!isPending && deliveryNotes.length > 0 ? (deliveryNotes.map((deliveryNote) => {
            ...
            const isTransferred = linkedInvoices.length > 0 || visibleReturnNotes.length > 0;
            const hasUploads = (deliveryNote.uploads?.length ?? 0) > 0;
            return (<tr key={deliveryNoteId || deliveryNote.sequential} className="border-b border-zinc-200/70 text-sm text-zinc-700 transition hover:bg-zinc-50 dark:border-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-900/60">
                        <td className="px-4 py-4 align-middle">
                          <Checkbox checked={isSelected} disabled={!deliveryNoteId} className="border-zinc-400 dark:border-zinc-700" onCheckedChange={(checked) => {
                    if (!deliveryNoteId)
                        return;
                    setSelectedIds((current) => {
                        ...
                        <td className="px-4 py-4 align-middle">
                          <div className="flex flex-col">
                            <button type="button" className="font-semibold text-zinc-950 transition hover:text-primary dark:text-zinc-50 text-left" onClick={() => router.push(`${detailPathPrefix}/${deliveryNote.id}`)}>
                    ...
                    : '-'}
                            ...
                            {deliveryNote.dueDate && (<span className={cn('text-xs', mutedTextClassName)}>
                    ...
                    : tCommon('words.no_interlocutor')}
                        ...
                        <td className="px-4 py-4 align-middle">
                          <div className="flex items-center gap-2">
                            <span className={cn('inline-flex rounded-full border px-3 py-1 text-xs font-semibold', getStatusBadgeClassName(deliveryNote.status))}>
                              {deliveryNote.status ? tInvoicing(deliveryNote.status) : '-'}
                            </span>

                            {invoicedInvoices.length > 0 && (<span className="inline-flex rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-300">
                    ...
                    invoicedInvoices.length > 0) && (<CheckCircle2 className="h-4 w-4 text-emerald-500 shrink-0"/>)}
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-4 align-middle text-right">
                          <div className="flex items-center justify-end gap-1">
                            <Button variant="ghost" size="icon" className="h-8 w-8 rounded-sm text-zinc-500 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-zinc-900" onClick={() => openDeliveryNotePreview(deliveryNote)}>
                              <Eye className="h-4 w-4"/>
                            </Button>
                            <Button variant="ghost" size="icon" className="h-8 w-8 rounded-sm text-zinc-500 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-zinc-900" onClick={() => router.push(`${detailPathPrefix}/${deliveryNote.id}`)}>
                    ...
                    setStatusDialog(true);
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-64">
                                <DropdownMenuItem onClick={() => openDeliveryNotePreview(deliveryNote)}>
                    ...
                    setDetailsDialog(true);
                                ...
                                <DropdownMenuItem onClick={() => router.push(`${detailPathPrefix}/${deliveryNote.id}`)}>
                                  <Edit className="h-4 w-4 mr-2"/> {tCommon('commands.edit')}
                                </DropdownMenuItem>
                                <>
                                  {!isTransferred && (<DropdownMenuItem onClick={() => {
                        if (!setManagedDeliveryNote(deliveryNote))
                            return;
                        ...
                        setReturnNoteDialog(true);
                                  ...
                                  {linkedInvoices.map((invoice) => (<DropdownMenuItem key={`invoice-${invoice.id}`} onClick={() => {
                        setSelectedDeliveryNote(deliveryNote);
                        setDetailsDialog(true);
                    }}>
                                      <ExternalLink className="h-4 w-4 mr-2"/>
                                      {tInvoicing('deliveryNote.view_created_invoice', {
                        defaultValue: 'Voir la Facture Créée'
                    })}
                                    </DropdownMenuItem>))}
                                  {visibleReturnNotes.map((returnNote) => (<DropdownMenuItem key={`return-note-${returnNote.id}`} onClick={() => {
                        setSelectedDeliveryNote(deliveryNote);
                        setDetailsDialog(true);
                    }}>
                                      <ExternalLink className="h-4 w-4 mr-2"/>
                                      {tInvoicing('deliveryNote.view_created_return_note', {
                        defaultValue: 'Voir le Bon de Retour Créé'
                    })}
                                    </DropdownMenuItem>))}
                                </>
                                <DropdownMenuItem onClick={() => {
                    ...
                    setAttachmentDialog(true);
                                ...
                                {hasUploads && (<DropdownMenuItem onClick={() => handleDownloadWithAttachments(deliveryNote)}>
          ...
          <div className="flex flex-col gap-4 border-t border-zinc-200 px-4 py-4 text-sm text-zinc-600 dark:border-zinc-800 dark:text-zinc-400 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex items-center justify-end gap-2">
              <Button variant="outline" size="icon" className="h-10 w-10 rounded-sm" onClick={() => setPage(page - 1)} disabled={page <= 1 || totalPageCount === 0}>
                <ChevronLeft className="h-4 w-4"/>
              </Button>
              <Button variant="outline" size="icon" className="h-10 w-10 rounded-sm" onClick={() => setPage(page + 1)} disabled={page >= totalPageCount || totalPageCount === 0}>

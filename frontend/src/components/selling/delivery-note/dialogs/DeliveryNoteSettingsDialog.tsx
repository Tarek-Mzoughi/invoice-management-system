...
interface DeliveryNoteSettingsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  children: React.ReactNode;
}

export const DeliveryNoteSettingsDialog = ({
  open,
  onOpenChange,
  children
}: DeliveryNoteSettingsDialogProps) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[calc(100vw-2rem)] max-w-[760px] gap-0 p-0">
        <DialogHeader className="border-b border-zinc-200 px-6 py-5 pr-12 dark:border-zinc-800">
          <DialogTitle>{tInvoicing('deliveryNote.settings_dialog.title')}</DialogTitle>
          <DialogDescription>
            {tInvoicing('deliveryNote.settings_dialog.description')}
          </DialogDescription>
        </DialogHeader>

...
interface DeliveryNoteFinancialInformationProps {
  className?: string;
  total: number;
  subTotal?: number;
  discount?: number;
...
export const DeliveryNoteFinancialInformation = ({
  className,
  subTotal,
  total,
  currency,
  loading,
  edit = true
}: DeliveryNoteFinancialInformationProps) => {
  const { t: tInvoicing } = useTranslation('invoicing');

  const DeliveryNoteArticleManager = useDeliveryNoteArticleManager();
  const deliveryNoteManager = useDeliveryNoteManager();
  const currencySymbol = currency?.symbol || '$';
  const digitAfterComma = currency?.digitAfterComma || 3;
  const discount = deliveryNoteManager.discount ?? 0;
  const discountType = deliveryNoteManager.discountType;
      ...
      <div className="rounded-lg border border-zinc-200 bg-zinc-50/60 p-4 dark:border-zinc-800 dark:bg-zinc-900/30">
        <div className="flex items-center gap-3 py-1 text-sm">
          <Label className="mr-auto text-zinc-600 dark:text-zinc-300">
            {tInvoicing('deliveryNote.attributes.sub_total')}
          </Label>
        ...
        {DeliveryNoteArticleManager.taxSummary.map((ts) => {
          return (

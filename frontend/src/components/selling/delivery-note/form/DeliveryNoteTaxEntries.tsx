...
import { ArticleDeliveryNoteEntry, Currency, Tax } from '@/types';
...
interface DeliveryNoteTaxEntriesProps {
  className?: string;
...
export const DeliveryNoteTaxEntries: React.FC<DeliveryNoteTaxEntriesProps> = ({
  className,
  article,
  taxes,
  selectedTaxIds,
  ...
  return (
    <div className={cn('flex flex-col', className)}>
      {(!article?.articleDeliveryNoteEntryTaxes ||
        article?.articleDeliveryNoteEntryTaxes?.length == 0) && (
        <p className="font-thin text-sm m-1">{tInvoicing('article.no_applied_tax')}</p>
      )}
      {article.articleDeliveryNoteEntryTaxes?.map((appliedTax, i) => (

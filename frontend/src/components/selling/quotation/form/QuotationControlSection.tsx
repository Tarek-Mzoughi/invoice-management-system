...
export const QuotationControlSection = ({
  ...
  const { mutate: duplicateQuotation, isPending: isDuplicationPending } = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      await router.push(`${quotationPathPrefix}/${data.id}`);
  ...
  const { mutate: invoiceQuotation, isPending: isInvoicingPending } = useMutation({
    ...
    onError: (error) => {
      const message = getErrorMessage(
        'invoicing',
        error,
        tInvoicing('quotation.action_invoice_failure')
      );
      toast.error(message);
    }
  });
        ...
        <div className="space-y-2">
          ...
          <SelectShimmer isPending={loading}>
            <Select
              ...
              value={quotationManager?.bankAccount?.id?.toString() || ''}
            >
        ...
        <div className="space-y-2">
          ...
          <SelectShimmer isPending={loading}>
            <Select
              ...
              value={quotationManager?.currency?.id?.toString() || ''}
            >
              <SelectTrigger className="w-full">
  ...
  const dialogSettingsContent = (
    <div className="space-y-5">
      {showConfiguration && (
        <>
          <SettingCard title={tInvoicing('quotation.settings_dialog.currency_section')}>
            {currencySelector}
          </SettingCard>

          <SettingCard title={tInvoicing('quotation.settings_dialog.bank_section')}>
            <SettingRow
              label={tInvoicing('quotation.settings_dialog.display_bank_details')}
              hint={tInvoicing('quotation.settings_dialog.display_bank_details_hint')}
              checked={canReadTreasury && !controlManager.isBankAccountDetailsHidden}
              onCheckedChange={toggleBankDetails}
            />
            {bankAccountSelector}
      ...
      {showVisibility && (
        <>
          <SettingCard title={tInvoicing('quotation.settings_dialog.address_section')}>
            <SettingRow
              label={tInvoicing('quotation.attributes.invoicing_address')}
              hint={tInvoicing('quotation.settings_dialog.invoicing_address_hint')}
              checked={!controlManager.isInvoiceAddressHidden}
              onCheckedChange={toggleInvoiceAddress}
            />
            <SettingRow
              label={tInvoicing('quotation.attributes.delivery_address')}
              hint={tInvoicing('quotation.settings_dialog.delivery_address_hint')}
              checked={!controlManager.isDeliveryAddressHidden}
              onCheckedChange={toggleDeliveryAddress}
            />
          </SettingCard>

          <SettingCard title={tInvoicing('quotation.settings_dialog.content_section')}>
            <SettingRow
              label={tInvoicing('controls.article_description')}
              hint={tInvoicing('quotation.settings_dialog.article_description_hint')}
              checked={!controlManager.isArticleDescriptionHidden}
              onCheckedChange={toggleArticleDescription}
            />
            <SettingRow
              label={tInvoicing('quotation.attributes.general_condition')}
              hint={tInvoicing('quotation.settings_dialog.general_conditions_hint')}
              checked={!controlManager.isGeneralConditionsHidden}
              onCheckedChange={toggleGeneralConditions}
            />
          </SettingCard>
  ...
  return (
    <>
      ...
      <div className={cn(className, (isPanelLayout || isDialogLayout) && 'space-y-5')}>
        {showActions && (
          ...
          >
            ...
            >
              ...
              {/* Actions Dropdown */}
              {status && status !== QUOTATION_STATUS.Draft && (
                <DropdownMenu>
        ...
        {/* Invoice list */}
        {showInvoices && status === QUOTATION_STATUS.Invoiced && invoices.length != 0 && (

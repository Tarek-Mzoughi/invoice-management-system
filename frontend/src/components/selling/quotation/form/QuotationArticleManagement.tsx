...
const createQuotationEntryFromArticle = (
  ...
  return {
    ...
    articleQuotationEntryTaxes: uniqueTaxIds
...
export const QuotationArticleManagement: React.FC<QuotationArticleManagementProps> = ({
  ...
  const activityType = quotationManager.activityType || ACTIVITY_TYPE.SELLING;
  ...
  React.useEffect(() => {
    if ((quotationManager.discount || 0) > 0) {
  ...
  const handleArticleSelect = React.useCallback(
    (value: string) => {
      ...
      articleManager.add(createQuotationEntryFromArticle(selectedArticle, availableTaxes, activityType));
  ...
  const handleGlobalDiscountToggle = React.useCallback(
    (checked: boolean | 'indeterminate') => {
      ...
      if (!enabled) {
        quotationManager.set('discount', 0);
        quotationManager.set('discountType', DISCOUNT_TYPE.PERCENTAGE);
      }
    },
    [quotationManager]
  );
  const handleGlobalDiscountChange = React.useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const digitAfterComma = quotationManager.currency?.digitAfterComma || 3;
      const regex = new RegExp(`^\\d*(\\.\\d{0,${digitAfterComma}})?$`);
      if (regex.test(nextValue)) {
        quotationManager.set('discount', nextValue === '' ? 0 : parseFloat(nextValue));
      }
    },
    [quotationManager]
  );
      ...
      <div className="space-y-4">
        <div className="space-y-3">
          <h2 className="text-base font-medium text-zinc-950 dark:text-zinc-50">
            {tInvoicing('quotation.editor.article_prices_label')}
          </h2>
          <RadioGroup
            value={priceMode}
          ...
          >
            <div className="flex items-center gap-2">
              <RadioGroupItem value="HT" id="quotation-price-mode-ht" disabled={!edit} />
              <Label htmlFor="quotation-price-mode-ht">
                {tInvoicing('quotation.editor.price_excluding_tax')}
              </Label>
            </div>
            <div className="flex items-center gap-2">
              <RadioGroupItem value="TTC" id="quotation-price-mode-ttc" disabled={!edit} />
              <Label htmlFor="quotation-price-mode-ttc">
                {tInvoicing('quotation.editor.price_including_tax')}
              </Label>
            </div>
          </RadioGroup>
        </div>
                ...
                >
                  ...
                  {!loading && articleManager.articles.length === 0 && (
                    <div className="flex min-h-[96px] items-center justify-center px-4 py-6 text-sm text-muted-foreground">
                      {tInvoicing('quotation.editor.empty_article')}
                    </div>
                  )}
        ...
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto]">
          ...
          >
            <SelectTrigger className="h-10 bg-white text-left text-sm text-muted-foreground shadow-sm dark:bg-zinc-950">
              <SelectValue placeholder={tInvoicing('quotation.editor.select_article')} />
            </SelectTrigger>
            <SelectContent>
          ...
          <Button variant="outline" className="h-10" onClick={addNewItem}>
            {tInvoicing('quotation.editor.add_empty_line')}
          </Button>
        </div>
      )}
      ...
      <div className="rounded-lg border border-zinc-200 bg-zinc-50/40 p-4 dark:border-zinc-800 dark:bg-zinc-900/20">
        <div className="flex items-center gap-3">
          ...
          <Label htmlFor="quotation-global-discount">
            {tInvoicing('quotation.editor.add_global_discount')}
          </Label>
        </div>
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-[120px_minmax(0,1fr)_auto] lg:items-center">
            ...
            >
              ...
              <SelectContent align="start">
                ...
                <SelectItem value="AMOUNT">{quotationManager.currency?.symbol || '$'}</SelectItem>
              </SelectContent>
            </Select>
            ...
            <Input
              ...
              inputMode="decimal"
              disabled={!edit}
              value={quotationManager.discount ?? 0}

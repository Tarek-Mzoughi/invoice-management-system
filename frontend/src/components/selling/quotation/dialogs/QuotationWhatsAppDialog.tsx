...
interface QuotationWhatsAppDialogProps {
  open: boolean;
  onClose: () => void;
  quotation: Quotation | null;
}

export const QuotationWhatsAppDialog: React.FC<QuotationWhatsAppDialogProps> = ({
  open,
  onClose,
  quotation
}) => {
  ...
  React.useEffect(() => {
    if (quotation && open) {
      setPhone(quotation.interlocutor?.phone || '');
    }
  }, [quotation, open]);
  ...
  const handleSend = async () => {
    ...
    const message =
      `Bonjour ${clientName},\n\n` +
      `Veuillez trouver le devis ${reference} en piece jointe.\n\n` +
      'Cordialement.';

    setIsSending(true);
    try {
      const blob = await api.quotation.preview(quotation.id, 'template1');

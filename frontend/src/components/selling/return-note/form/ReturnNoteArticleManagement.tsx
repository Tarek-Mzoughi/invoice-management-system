...
import { ReturnNoteArticleItem } from './ReturnNoteArticleItem';
...
const createReturnNoteEntryFromArticle = (
  ...
  return {
    ...
    articleReturnNoteEntryTaxes: uniqueTaxIds
      .map((taxId) => taxes.find((tax) => tax.id === taxId))
      .filter((tax): tax is Tax => Boolean(tax))
      .map((tax) => ({ tax }))
  };
};
interface ReturnNoteArticleManagementProps {
  className?: string;
  taxes: Tax[];
...
export const ReturnNoteArticleManagement: React.FC<ReturnNoteArticleManagementProps> = ({
  className,
  taxes = [],
  ...
  const activityType = returnNoteManager.activityType || ACTIVITY_TYPE.SELLING;
  ...
  React.useEffect(() => {
    if ((returnNoteManager.discount || 0) > 0) {
      setIsGlobalDiscountEnabled(true);
    }
  }, [returnNoteManager.discount]);
  ...
  const handleArticleSelect = React.useCallback(
    (value: string) => {
      ...
      articleManager.add(createReturnNoteEntryFromArticle(selectedArticle, availableTaxes, activityType));
  ...
  const handleGlobalDiscountToggle = React.useCallback(
    (checked: boolean | 'indeterminate') => {
      ...
      if (!enabled) {
        returnNoteManager.set('discount', 0);
        returnNoteManager.set('discountType', DISCOUNT_TYPE.PERCENTAGE);
      }
    },
    [returnNoteManager]
  );
  const handleGlobalDiscountChange = React.useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const digitAfterComma = returnNoteManager.currency?.digitAfterComma || 3;
      const regex = new RegExp(`^\\d*(\\.\\d{0,${digitAfterComma}})?$`);
      if (regex.test(nextValue)) {
        returnNoteManager.set('discount', nextValue === '' ? 0 : parseFloat(nextValue));
      }
    },
    [returnNoteManager]
  );
  const globalDiscountType =
    returnNoteManager.discountType === DISCOUNT_TYPE.PERCENTAGE ? 'PERCENTAGE' : 'AMOUNT';
  return (
        ...
        {showPrices && (
          <div className="space-y-3">
            <h2 className="text-base font-medium text-zinc-950 dark:text-zinc-50">
              {tInvoicing('returnNote.editor.article_prices_label')}
            </h2>
            <RadioGroup
              value={priceMode}
            ...
            >
              <div className="flex items-center gap-2">
                <RadioGroupItem value="HT" id="returnNote-price-mode-ht" disabled={!edit} />
                <Label htmlFor="returnNote-price-mode-ht">
                  {tInvoicing('returnNote.editor.price_excluding_tax')}
                </Label>
              </div>
              <div className="flex items-center gap-2">
                <RadioGroupItem value="TTC" id="returnNote-price-mode-ttc" disabled={!edit} />
                <Label htmlFor="returnNote-price-mode-ttc">
                  {tInvoicing('returnNote.editor.price_including_tax')}
                </Label>
              </div>
            </RadioGroup>
          </div>
                ...
                >
                  ...
                  {!loading && articleManager.articles.length === 0 && (
                    <div className="flex min-h-[96px] items-center justify-center px-4 py-6 text-sm text-muted-foreground">
                      {tInvoicing('returnNote.editor.empty_article')}
                    </div>
                  )}
        ...
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto]">
          ...
          >
            <SelectTrigger className="h-10 bg-white text-left text-sm text-muted-foreground shadow-sm dark:bg-zinc-950">
              <SelectValue placeholder={tInvoicing('returnNote.editor.select_article')} />
            </SelectTrigger>
            <SelectContent>
              {selectableArticles.length === 0 ? (
                <SelectItem value="__empty" disabled>
                  {tInvoicing('returnNote.editor.no_article_available')}
                </SelectItem>
              ) : (
          ...
          <Button
            ...
            onClick={addNewItem}
          >
            {tInvoicing('returnNote.editor.add_empty_line')}
          </Button>
        </div>
      )}
      ...
      {showPrices && (
        <div className="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
          <div className="flex items-center gap-3">
            <Checkbox
              id="returnNote-global-discount"
              checked={isGlobalDiscountEnabled}
              disabled={!edit}
              onCheckedChange={handleGlobalDiscountToggle}
            />
            <Label htmlFor="returnNote-global-discount" className="text-sm font-medium">
              {tInvoicing('returnNote.editor.global_discount')}
            </Label>
          </div>
          ...
          {isGlobalDiscountEnabled && (
            <div className="mt-4 grid gap-3 md:grid-cols-[140px_minmax(0,220px)]">
              ...
              >
                ...
                <SelectContent align="start">
                  ...
                  <SelectItem value="AMOUNT">
                    {returnNoteManager.currency?.symbol || '$'}
                  </SelectItem>
                </SelectContent>
              </Select>
              ...
              <Input
                ...
                inputMode="decimal"
                disabled={!edit}
                value={returnNoteManager.discount ?? ''}

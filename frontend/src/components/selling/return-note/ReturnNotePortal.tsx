...
import { ChevronDown, ChevronLeft, ChevronRight, ChevronUp, Download, Edit, ExternalLink, Eye, FileSpreadsheet, FileText, Mail, MessageCircle, MoreHorizontal, Paperclip, PencilLine, Plus, Search, Settings2, Trash2, User2, X } from 'lucide-react';
import { api } from '@/api';
...
export const ReturnNotePortal: React.FC<ReturnNoteMainProps> = ({ className, scope = 'selling', rootPath = '/selling', listPath = '/selling/return-notes', newPath = '/selling/new-return-note', detailPathPrefix = '/selling/return-note', linkedInvoicePathPrefix = '/selling/invoice' }) => {
    ...
    const { router, tCommon, tInvoicing, i18n, dateLocale, numberLocale, partnerLabel, partnerPlaceholder, allPartnersLabel, setIntro, clearIntro, setRoutes, clearRoutes, setFloating, clearFloating, page, setPage, size, setSize, totalPageCount, totalResultCount, searchTerm, filters, isAdvancedFiltersOpen, setIsAdvancedFiltersOpen, hasActiveFilters, handleFilterChange, handleSearchChange, handleResetFilters, selectedIds, setSelectedIds, pageIds, isAllPageSelected, isPartiallySelected, items: returnNotes, firms, error, refetchList: refetchReturnNotes, deleteDialog, setDeleteDialog, duplicateDialog, setDuplicateDialog, downloadDialog, setDownloadDialog, previewDialog, previewBlob, closePreviewDialog, removeItem: removeReturnNote, duplicateItem: duplicateReturnNote, downloadItem: downloadReturnNote, loadPreview, isPending: basePending, isDeletePending, isDuplicationPending, isDownloadPending, isPreviewPending } = portal;
    ...
    const setManagedReturnNote = React.useCallback((returnNote: ReturnNote) => {
        if (!returnNote.id)
            return false;
    ...
    const selectedReturnNotes = React.useMemo(() => returnNotes.filter((returnNote) => returnNote.id ? selectedIds.includes(returnNote.id) : false), [returnNotes, selectedIds]);
    const buildExportRows = React.useCallback((notes: ReturnNote[]) => notes.map((returnNote) => ({
    ...
    const openReturnNotePreview = React.useCallback((returnNote: ReturnNote) => {
        if (!returnNote.id || !setManagedReturnNote(returnNote))
            return;
    ...
    const closeReturnNoteDetailsThen = React.useCallback((callback: () => void) => runAfterOverlayClose(() => setDetailsDialog(false), callback), [runAfterOverlayClose]);
    const buildExportFileName = React.useCallback((extension: 'pdf' | 'xlsx') => `return-notes-${new Date().toISOString().slice(0, 10)}.${extension}`, []);
    const buildGeneratedAtLabel = React.useCallback(() => new Intl.DateTimeFormat(i18n.language === 'fr' ? 'fr-FR' : 'en-US', {
        dateStyle: 'medium',
        timeStyle: 'short'
    }).format(new Date()), [i18n.language]);
    const handleBulkExportPdf = React.useCallback(async () => {
        if (selectedReturnNotes.length === 0)
            return;
        try {
            await exportReturnNotesToPdf({
                rows: buildExportRows(selectedReturnNotes),
                labels: {
                    reference: tInvoicing('returnNote.attributes.number'),
                    client: tInvoicing('returnNote.attributes.customer'),
    ...
    const handleDownloadWithAttachments = React.useCallback(async (returnNote: ReturnNote) => {
        try {
            if (!returnNote.id)
                return;
            ...
            toast.error(getErrorMessage('invoicing', error as Error, tInvoicing('returnNote.action_download_failure')));
        }
    }, [tInvoicing]);
    ...
    React.useEffect(() => {
        ...
        setFloating?.(<div className="flex items-center gap-2">
        {selectedIds.length > 0 && (<DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="h-11 rounded-md border-zinc-200 px-5 dark:border-zinc-800">
            ...
            <DropdownMenuContent align="end" className="w-64">
              ...
              <DropdownMenuItem onClick={() => setBulkDeleteDialog(true)} className="gap-2 text-rose-600 focus:bg-rose-50 focus:text-rose-600 dark:focus:bg-rose-950/50">
    ...
    const { mutate: invoiceReturnNote, isPending: isInvoicingPending } = useMutation({
        ...
        onError: (error) => {
            toast.error(getErrorMessage('invoicing', error, tInvoicing('transformations.invoice.failure')));
        }
    });
    ...
    const openLinkedDocument = React.useCallback((type: string, id: number) => {
        if (type === 'invoice') {
    ...
    return (<>
      <ReturnNoteBulkDeleteDialog count={selectedIds.length} open={bulkDeleteDialog} onDelete={handleBulkDelete} isPending={isBulkDeleting} onClose={() => setBulkDeleteDialog(false)}/>

      <ReturnNoteDeleteDialog id={returnNoteManager?.id} sequential={returnNoteManager?.sequential || ''} scope={scope} open={deleteDialog} deleteReturnNote={() => {
      ...
      <ReturnNoteDuplicateDialog id={returnNoteManager?.id || 0} sequential={returnNoteManager?.sequential || ''} scope={scope} open={duplicateDialog} duplicateReturnNote={(includeFiles: boolean) => {
      ...
      <ReturnNoteDownloadDialog id={returnNoteManager?.id || 0} open={downloadDialog} downloadReturnNote={(template: string) => {
      ...
      {<ReturnNoteInvoiceDialog id={returnNoteManager?.id || 0} status={returnNoteManager?.status} sequential={returnNoteManager?.sequential} open={invoiceDialog} isInvoicePending={isInvoicingPending} invoice={(id: number, createInvoice: boolean) => {
                invoiceReturnNote({ id, createInvoice });
            }} onClose={() => setInvoiceDialog(false)}/>}

      <ReturnNoteDetailsDialog open={detailsDialog} onClose={() => setDetailsDialog(false)} returnNote={selectedReturnNote} detailPathPrefix={detailPathPrefix} scope={scope} onPreview={() => {
            if (!selectedReturnNote)
                return;
            closeReturnNoteDetailsThen(() => openReturnNotePreview(selectedReturnNote));
        }} onDownload={() => {
            if (!selectedReturnNote || !setManagedReturnNote(selectedReturnNote))
                return;
            closeReturnNoteDetailsThen(() => setDownloadDialog(true));
        }} onDownloadWithAttachments={selectedReturnNote?.uploads?.length
            ? () => {
                if (!selectedReturnNote)
                    return;
                closeReturnNoteDetailsThen(() => handleDownloadWithAttachments(selectedReturnNote));
            }
            : undefined} onDuplicate={() => {
            if (!selectedReturnNote || !setManagedReturnNote(selectedReturnNote))
                return;
            ...
            if (!selectedReturnNote || !setManagedReturnNote(selectedReturnNote))
                return;
            closeReturnNoteDetailsThen(() => setInvoiceDialog(true));
        }} onEmail={() => {
            if (!selectedReturnNote)
                return;
      ...
      <ReturnNoteEmailDialog open={emailDialog} onClose={() => setEmailDialog(false)} returnNote={selectedReturnNote} scope={scope}/>

      <ReturnNoteWhatsAppDialog open={whatsAppDialog} onClose={() => setWhatsAppDialog(false)} returnNote={selectedReturnNote} scope={scope}/>

      <ReturnNoteAttachmentDialog open={attachmentDialog} onClose={() => setAttachmentDialog(false)} returnNote={selectedReturnNote} scope={scope} onSaved={async () => {
      ...
      <DocumentPreviewDialog open={previewDialog} loading={isPreviewPending} previewBlob={previewBlob} filename={`${returnNoteManager.sequential || `return-note-${returnNoteManager.id || 'preview'}`}.pdf`} title={tCommon('commands.preview')} onClose={closePreviewDialog}/>
            ...
            <div className="space-y-2">
              ...
              <Select value={filters.status} onValueChange={(value) => handleFilterChange('status', value)}>
                ...
                <SelectContent>
                  ...
                  {RETURN_NOTE_STATUS_OPTIONS.map((status) => (<SelectItem key={status} value={status}>
                      {tInvoicing(status)}
                    </SelectItem>))}
                </SelectContent>
              </Select>
          ...
          <div className="min-h-0 flex-1 overflow-auto">
            ...
            setSelectedIds([]);
                  ...
                  <th className="px-4 py-3.5 font-medium">
                    {tInvoicing('returnNote.attributes.number')}
                  </th>
                  <th className="px-4 py-3.5 font-medium">
                    {tInvoicing('returnNote.attributes.customer')}
                  </th>
                  <th className="px-4 py-3.5 font-medium">
              ...
              <tbody>
                {!isPending && returnNotes.length > 0 ? (returnNotes.map((returnNote) => {
            ...
            return (<tr key={returnNoteId || returnNote.sequential} className="border-b border-zinc-200/70 text-sm text-zinc-700 transition hover:bg-zinc-50 dark:border-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-900/60">
                        <td className="px-4 py-4 align-middle">
                          <Checkbox checked={isSelected} disabled={!returnNoteId} className="border-zinc-400 dark:border-zinc-700" onCheckedChange={(checked) => {
                    if (!returnNoteId)
                        return;
                    setSelectedIds((current) => {
                        ...
                        <td className="px-4 py-4 align-middle">
                          <div className="flex flex-col">
                            <button type="button" className="text-left font-semibold text-zinc-950 transition hover:text-primary dark:text-zinc-50" onClick={() => router.push(`${detailPathPrefix}/${returnNote.id}`)}>
                    ...
                    : '-'}
                            ...
                            {returnNote.dueDate && (<span className={cn('text-xs', mutedTextClassName)}>
                    ...
                    : tCommon('words.no_interlocutor')}
                        ...
                        <td className="px-4 py-4 align-middle">
                          <span className={cn('inline-flex rounded-full border px-3 py-1 text-xs font-semibold', getStatusBadgeClassName(returnNote.status))}>
                        ...
                        <td className="px-4 py-4 align-middle text-right">
                          <div className="flex items-center justify-end gap-1">
                            <Button variant="ghost" size="icon" className="h-8 w-8 rounded-sm text-zinc-500 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-zinc-900" onClick={() => openReturnNotePreview(returnNote)}>
                              <Eye className="h-4 w-4"/>
                            </Button>
                            <Button variant="ghost" size="icon" className="h-8 w-8 rounded-sm text-zinc-500 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-zinc-900" onClick={() => router.push(`${detailPathPrefix}/${returnNote.id}`)}>
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-64">
                                <DropdownMenuItem className="cursor-pointer gap-2 px-3 py-2" onClick={() => openReturnNotePreview(returnNote)}>
                                  <Eye className="h-4 w-4"/>
                                  {tCommon('commands.view')}
                                </DropdownMenuItem>
                                <DropdownMenuItem className="cursor-pointer gap-2 px-3 py-2" onClick={() => openReturnNoteDetails(returnNote)}>
                                  <ExternalLink className="h-4 w-4"/>
                                  {tCommon('commands.details')}
                                </DropdownMenuItem>
                                <DropdownMenuItem className="cursor-pointer gap-2 px-3 py-2" onClick={() => router.push(`${detailPathPrefix}/${returnNote.id}`)}>
                                  <Edit className="h-4 w-4"/>
                                  {tCommon('commands.edit')}
                                </DropdownMenuItem>
                                <DropdownMenuItem className="cursor-pointer gap-2 px-3 py-2" onClick={() => {
                    if (!setManagedReturnNote(returnNote))
                        return;
                    ...
                    setAttachmentDialog(true);
                                ...
                                {hasUploads && (<DropdownMenuItem className="cursor-pointer gap-2 px-3 py-2" onClick={() => handleDownloadWithAttachments(returnNote)}>
                                    <Paperclip className="h-4 w-4"/>
                                    {tInvoicing('returnNote.action_download_with_attachments')}
                                  </DropdownMenuItem>)}
                                <DropdownMenuItem className="cursor-pointer gap-2 px-3 py-2 text-rose-600 focus:bg-rose-50 focus:text-rose-600 dark:focus:bg-rose-950/50" onClick={() => {
                    if (!setManagedReturnNote(returnNote))
                        return;

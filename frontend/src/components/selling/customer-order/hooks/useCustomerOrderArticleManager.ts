import { ArticleCustomerOrderEntry, Tax } from '@/types';
import { DISCOUNT_TYPE } from '@/types/enums/discount-types';
import { v4 as uuidv4 } from 'uuid';
import { create } from 'zustand';

type CustomerOrderPseudoItem = {
  id: string;
  article: ArticleCustomerOrderEntry & { total?: number };
};

export type CustomerOrderArticleManager = {
  articles: CustomerOrderPseudoItem[];
  taxSummary: { tax: Tax; amount: number }[];
  add: (article?: ArticleCustomerOrderEntry) => void;
  update: (id: string, article: ArticleCustomerOrderEntry) => void;
  delete: (id: string) => void;
  setArticles: (articles: ArticleCustomerOrderEntry[]) => void;
  reset: () => void;
  getArticles: () => (ArticleCustomerOrderEntry & { total: number })[];
  //feature
  removeArticleDescription: () => void;
};

const calculateForCustomerOrder = (article: ArticleCustomerOrderEntry) => {
  ...
  if (article?.articleCustomerOrderEntryTaxes) {
    for (const entry of article?.articleCustomerOrderEntryTaxes) {
      if (entry?.tax?.isRate) {
...
export const useCustomerOrderArticleManager = create<CustomerOrderArticleManager>()((set, get) => ({
  articles: [],
  taxSummary: [],

  add: (article: ArticleCustomerOrderEntry = {}) => {
    const { subTotal, total } = calculateForCustomerOrder(article);
  ...
  update: (id: string, article: ArticleCustomerOrderEntry) => {
    const { subTotal, total } = calculateForCustomerOrder(article);
  ...
  setArticles: (articles: ArticleCustomerOrderEntry[]) => {
    set({
  ...
  getArticles: () => {
    return get().articles.map((item) => {
      const { subTotal, total } = calculateForCustomerOrder(item.article);

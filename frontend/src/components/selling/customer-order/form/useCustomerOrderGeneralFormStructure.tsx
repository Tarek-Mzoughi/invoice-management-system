...
export const useCustomerOrderGeneralFormStructure = ({
  ...
  const documentLabels = useScopedDocumentLabels('customerOrder', 'selling');
  const firmLabel = tInvoicing('customerOrder.attributes.customer');
  const firmPlaceholder = tInvoicing('customerOrder.associate_customer');
  ...
  const referenceField: Field<TextFieldProps> | null = null;

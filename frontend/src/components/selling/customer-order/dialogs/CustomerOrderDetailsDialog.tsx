...
import { MoreHorizontal, Eye, ExternalLink, Edit, FileText, ShoppingCart, Truck, Download, Mail, MessageCircle, RefreshCw, Copy, Paperclip, Trash2, LayoutDashboard, ChevronDown, Package } from 'lucide-react';
import { CUSTOMER_ORDER_STATUS, CustomerOrder, DELIVERY_NOTE_STATUS, INVOICE_STATUS } from '@/types';
...
export const CustomerOrderDetailsDialog: React.FC<CustomerOrderDetailsDialogProps> = ({ open, onClose, customerOrder, detailPathPrefix, scope, onPreview, onDownload, onDuplicate, onStatusChange, onDelete, onInvoice, onCustomerOrder, onDeliveryNote, onEmail, onWhatsApp, onAttachment, onLinkedDocumentPreview }) => {
          ...
          <div className="flex items-center gap-2">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-64 p-1">
                ...
                <DropdownMenuItem onClick={() => router.push(`${detailPathPrefix}/${customerOrder.id}`)} className="gap-2 px-3 py-2 cursor-pointer">
            ...
            0 ? (<>
                    <DropdownMenuItem onClick={onInvoice} className="gap-2 px-3 py-2 cursor-pointer">
                      <FileText className="h-4 w-4"/>{' '}
                      {`${tCommon('commands.transform_to')} ${invoiceLabels.singular}`}
                    </DropdownMenuItem>

                    <DropdownMenuItem onClick={onDeliveryNote} className="gap-2 px-3 py-2 cursor-pointer">
                      <Truck className="h-4 w-4"/>{' '}
                      {`${tCommon('commands.transform_to')} ${deliveryNoteLabels.singular}`}
                    </DropdownMenuItem>
                  </>) : (<>
                    {customerOrder.invoices && customerOrder.invoices.length > 0 && (<DropdownMenuItem onClick={() => onLinkedDocumentPreview('invoice', customerOrder.invoices![0].id!)} className="gap-2 px-3 py-2 cursor-pointer">
                        <FileText className="h-4 w-4 text-amber-500"/>{' '}
                        {tInvoicing('customerOrder.view_created_invoice')}
                      </DropdownMenuItem>)}
                    {false}
                    {customerOrder.deliveryNotes && customerOrder.deliveryNotes.length > 0 && (<DropdownMenuItem onClick={() => onLinkedDocumentPreview('delivery-note', customerOrder.deliveryNotes![0].id!)} className="gap-2 px-3 py-2 cursor-pointer">
                ...
                <DropdownMenuItem onClick={onStatusChange} className="gap-2 px-3 py-2 cursor-pointer">
                ...
                <DropdownMenuItem onClick={onDelete} className="gap-2 px-3 py-2 cursor-pointer text-rose-600 focus:text-rose-600 focus:bg-rose-50 dark:focus:bg-rose-950/50">
                  <Trash2 className="h-4 w-4"/> {tCommon('commands.delete')}
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            <Button variant="outline" onClick={onClose} className="h-9 border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900">
              Fermer
            </Button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-8 space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <Badge variant={getStatusBadgeVariant(displayStatus) as any} className={cn('px-2.5 py-0.5 text-xs font-semibold', getStatusBadgeClassName(displayStatus))}>
                {displayStatus ? tInvoicing(displayStatus) : '-'}
              </Badge>

              {hasEffectiveInvoice && (<Badge className="bg-amber-100 text-amber-700 hover:bg-amber-100 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800">
                  {tInvoicing('customerOrder.status.invoiced')}
                </Badge>)}
              {hasEffectiveDelivery && (<Badge className="bg-blue-100 text-blue-700 hover:bg-blue-100 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800">
            ...
            <div className="pt-2">
              <button onClick={() => customerOrder.firmId && router.push(`/contacts/firm/${customerOrder.firmId}`)} className="text-blue-600 hover:text-blue-700 hover:underline font-semibold transition-colors text-left">
          ...
          <div className="flex flex-wrap gap-3">
            <Button variant="outline" onClick={onDownload} className="gap-2 h-10 border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 shadow-sm dark:hover:bg-zinc-900">
              <Download className="h-4 w-4 text-zinc-500"/> Télécharger le Document
            </Button>
            <Button variant="outline" onClick={() => toast.info('Graphique bientôt disponible...')} className="gap-2 h-10 border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 shadow-sm dark:hover:bg-zinc-900">
              <LayoutDashboard className="h-4 w-4 text-zinc-500"/> Voir le Graphique
            </Button>
          </div>

          <div className="flex justify-end">
            <Button variant="outline" onClick={() => toast.info('Stock bientôt disponible...')} className="gap-2 h-10 border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 shadow-sm dark:hover:bg-zinc-900">
            ...
            <div className="rounded-md border border-zinc-100 dark:border-zinc-900 overflow-hidden">
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
                  {/* Invoices */}
                  {customerOrder.invoices?.map((invoice) => (<tr key={invoice.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/50 transition-colors">
                ...
                : '-'}
                      ...
                      <td className="px-4 py-3 text-right">
                        <Button variant="ghost" size="sm" className="text-blue-600 hover:text-blue-700 hover:bg-blue-50 dark:text-blue-400 dark:hover:bg-blue-900/20 font-semibold" onClick={() => onLinkedDocumentPreview('invoice', invoice.id!)}>
                          Voir le Document
                        </Button>
                      </td>
                    </tr>))}
                  ...
                  {/* Delivery Notes */}
                  {customerOrder.deliveryNotes?.map((dn) => (<tr key={dn.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/50 transition-colors">
                      <td className="px-4 py-3 font-medium text-zinc-700 dark:text-zinc-300">
                ...
                : '-'}
                      ...
                      <td className="px-4 py-3 text-right">
                        <Button variant="ghost" size="sm" className="text-blue-600 hover:text-blue-700 hover:bg-blue-50 dark:text-blue-400 dark:hover:bg-blue-900/20 font-semibold" onClick={() => onLinkedDocumentPreview('delivery-note', dn.id!)}>
                          Voir le Document
                        </Button>
                      </td>
                    </tr>))}

...
const useGoodsIssueNoteSocket = (
  label: Sequences = Sequences.GOODS_ISSUE_NOTE,
  ...
  React.useEffect(() => {
    ...
    const handleConnect = () => {
      if (!hasJoinedRef.current) {
        socket.emit('joinRoom', SocketRoom.GOODS_ISSUE_NOTE);
    ...
    socket.on('goods-issue-note-sequence-updated', (data) => {
    ...
    return () => {
      if (socket && hasJoinedRef.current) {
        socket.emit('leaveRoom', SocketRoom.GOODS_ISSUE_NOTE);
        console.log('Left room: GOODS_ISSUE_NOTE_SEQUENCE');
        hasJoinedRef.current = false;
      }

      socket.off('connect', handleConnect);
      socket.off('goods-issue-note-sequence-updated');
...
export default useGoodsIssueNoteSocket;

import { ArticleGoodsIssueNoteEntry, Tax } from '@/types';
import { DISCOUNT_TYPE } from '@/types/enums/discount-types';
import { v4 as uuidv4 } from 'uuid';
import { create } from 'zustand';

type GoodsIssueNotePseudoItem = {
  id: string;
  article: ArticleGoodsIssueNoteEntry & { total?: number };
};

export type GoodsIssueNoteArticleManager = {
  articles: GoodsIssueNotePseudoItem[];
  taxSummary: { tax: Tax; amount: number }[];
  add: (article?: ArticleGoodsIssueNoteEntry) => void;
  update: (id: string, article: ArticleGoodsIssueNoteEntry) => void;
  delete: (id: string) => void;
  setArticles: (articles: ArticleGoodsIssueNoteEntry[]) => void;
  reset: () => void;
  getArticles: () => (ArticleGoodsIssueNoteEntry & { total: number })[];
  //feature
  removeArticleDescription: () => void;
};

const calculateForGoodsIssueNote = (article: ArticleGoodsIssueNoteEntry) => {
  ...
  if (article?.articleGoodsIssueNoteEntryTaxes) {
    for (const entry of article?.articleGoodsIssueNoteEntryTaxes) {
      if (entry?.tax?.isRate) {
...
export const useGoodsIssueNoteArticleManager = create<GoodsIssueNoteArticleManager>()(
  (set, get) => ({
    articles: [],
    taxSummary: [],

    add: (article: ArticleGoodsIssueNoteEntry = {}) => {
      const { subTotal, total } = calculateForGoodsIssueNote(article);
    ...
    update: (id: string, article: ArticleGoodsIssueNoteEntry) => {
      const { subTotal, total } = calculateForGoodsIssueNote(article);
    ...
    setArticles: (articles: ArticleGoodsIssueNoteEntry[]) => {
      set({
    ...
    getArticles: () => {
      return get().articles.map((item) => {
        const { subTotal, total } = calculateForGoodsIssueNote(item.article);

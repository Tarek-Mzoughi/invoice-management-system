...
import { ArticleGoodsIssueNoteEntry, Currency, GoodsIssueNoteTaxEntry, Tax } from '@/types';
import {
  Select,
  SelectTrigger,
  SelectContent,
...
interface GoodsIssueNoteArticleItemProps {
  className?: string;
  article: ArticleGoodsIssueNoteEntry;
  onChange: (item: ArticleGoodsIssueNoteEntry) => void;
...
export const GoodsIssueNoteArticleItem: React.FC<GoodsIssueNoteArticleItemProps> = ({
  className,
  article,
  onChange,
  taxes,
  ...
  const selectedTaxIds = React.useMemo(
    () => (canReadTaxes ? article.articleGoodsIssueNoteEntryTaxes?.map((taxEntry) => taxEntry.tax?.id) || [] : []),
    [article.articleGoodsIssueNoteEntryTaxes, canReadTaxes]
  );
  const primaryTax = canReadTaxes ? article.articleGoodsIssueNoteEntryTaxes?.[0] : undefined;
  const extraTaxes = React.useMemo(
    () => (canReadTaxes ? article.articleGoodsIssueNoteEntryTaxes?.slice(1) || [] : []),
    [article.articleGoodsIssueNoteEntryTaxes, canReadTaxes]
  );

  const getAvailableTaxes = React.useCallback(
    (index: number) => {
      const currentTaxId = article.articleGoodsIssueNoteEntryTaxes?.[index]?.tax?.id;
      return canReadTaxes ? taxes.filter((tax) => !selectedTaxIds.includes(tax.id) || tax.id === currentTaxId) : [];
    },
    [article.articleGoodsIssueNoteEntryTaxes, canReadTaxes, selectedTaxIds, taxes]
  );

  const updateArticle = React.useCallback(
    (partial: Partial<ArticleGoodsIssueNoteEntry>) => {
      onChange({
        ...article,
        ...partial
      });
  ...
  const handleTaxChange = (value: string, index: number) => {
    ...
    const updatedTaxes = [...(article.articleGoodsIssueNoteEntryTaxes || [])];
    ...
    updateArticle({ articleGoodsIssueNoteEntryTaxes: updatedTaxes });
  };

  const handleTaxDelete = (index: number) => {
    if (!canReadTaxes) return;
    const updatedTaxes = article.articleGoodsIssueNoteEntryTaxes?.filter((_, i) => i !== index);
    updateArticle({ articleGoodsIssueNoteEntryTaxes: updatedTaxes });
  };

  const handleAddTax = () => {
    if (!canReadTaxes) return;
    if ((article.articleGoodsIssueNoteEntryTaxes?.length || 0) >= taxes.length) {
      toast.info(tInvoicing('goodsIssueNote.errors.surpassed_tax_limit'));
      return;
    }
      ...
      <div className="space-y-3">
        <div className="space-y-2">
          ...
          <Input
            className="h-10"
            disabled={!edit}
            placeholder={tInvoicing('goodsIssueNote.editor.product_placeholder')}
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-sm">
            ...
            <Label htmlFor={descriptionToggleId}>
              {tInvoicing('goodsIssueNote.editor.show_description')}
            </Label>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-sm">
              ...
              <Label htmlFor={discountToggleId}>
                {tInvoicing('goodsIssueNote.editor.apply_discount')}
              </Label>
            </div>
            ...
            {tInvoicing('article.no_applied_tax')}
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  >
                    <SelectTrigger className="h-10">
                      <SelectValue placeholder={tInvoicing('goodsIssueNote.editor.select_taxes')} />
                    </SelectTrigger>
                    <SelectContent align="start">
                ...
                {extraTaxes.map((taxEntry, index) => (
                  ...
                  >
                    ...
                    >
                      <SelectTrigger className="h-10">
                        <SelectValue
                          placeholder={tInvoicing('goodsIssueNote.editor.select_taxes')}
                        />
                      </SelectTrigger>
                      <SelectContent align="start">
              ...
              <div className="space-y-1 text-sm text-zinc-600 dark:text-zinc-300">
                {(article.articleGoodsIssueNoteEntryTaxes || []).length === 0 ? (
                  <p className="text-muted-foreground">
                    {tInvoicing('goodsIssueNote.editor.no_taxes')}
                  </p>
                ) : (
                  article.articleGoodsIssueNoteEntryTaxes?.map((taxEntry, index) => (

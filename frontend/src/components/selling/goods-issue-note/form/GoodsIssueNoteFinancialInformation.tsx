...
interface GoodsIssueNoteFinancialInformationProps {
  className?: string;
  total: number;
  subTotal?: number;
  discount?: number;
...
export const GoodsIssueNoteFinancialInformation = ({
  className,
  subTotal,
  total,
  currency,
  loading,
  edit = true
}: GoodsIssueNoteFinancialInformationProps) => {
  const { t: tInvoicing } = useTranslation('invoicing');

  const GoodsIssueNoteArticleManager = useGoodsIssueNoteArticleManager();
  const goodsIssueNoteManager = useGoodsIssueNoteManager();
  const currencySymbol = currency?.symbol || '$';
  const digitAfterComma = currency?.digitAfterComma || 3;
  const discount = goodsIssueNoteManager.discount ?? 0;
  const discountType = goodsIssueNoteManager.discountType;
      ...
      <div className="rounded-lg border border-zinc-200 bg-zinc-50/60 p-4 dark:border-zinc-800 dark:bg-zinc-900/30">
        <div className="flex items-center gap-3 py-1 text-sm">
          <Label className="mr-auto text-zinc-600 dark:text-zinc-300">
            {tInvoicing('goodsIssueNote.attributes.sub_total')}
          </Label>
        ...
        {GoodsIssueNoteArticleManager.taxSummary.map((ts) => {
          return (
        ...
        {!edit && discount > 0 && (
          <div className="flex flex-col w-full border-t border-zinc-200 pt-4 dark:border-zinc-800">
            <div className="flex items-center gap-3 text-sm">
              <Label className="mr-auto text-zinc-600 dark:text-zinc-300">
                {tInvoicing('goodsIssueNote.attributes.discount')}
              </Label>
              <Label className="ml-auto text-right" isPending={loading || false}>

...
export const InvoiceArticleItem: React.FC<InvoiceArticleItemProps> = ({
  ...
  const selectedTaxIds = React.useMemo(
    () => (canReadTaxes ? article.articleInvoiceEntryTaxes?.map((taxEntry) => taxEntry.tax?.id) || [] : []),
    [article.articleInvoiceEntryTaxes, canReadTaxes]
  );
  const primaryTax = canReadTaxes ? article.articleInvoiceEntryTaxes?.[0] : undefined;
  const extraTaxes = React.useMemo(
    () => (canReadTaxes ? article.articleInvoiceEntryTaxes?.slice(1) || [] : []),
    [article.articleInvoiceEntryTaxes, canReadTaxes]
  );

  const getAvailableTaxes = React.useCallback(
    (index: number) => {
      const currentTaxId = article.articleInvoiceEntryTaxes?.[index]?.tax?.id;
      return canReadTaxes ? taxes.filter((tax) => !selectedTaxIds.includes(tax.id) || tax.id === currentTaxId) : [];
    },
    [article.articleInvoiceEntryTaxes, canReadTaxes, selectedTaxIds, taxes]
  );

  const updateArticle = React.useCallback(
    (partial: Partial<ArticleInvoiceEntry>) => {
      onChange({
        ...article,
        ...partial
      });
  ...
  const handleTaxChange = (value: string, index: number) => {
    ...
    updateArticle({ articleInvoiceEntryTaxes: updatedTaxes });
  };
  ...
  const handleAddTax = () => {
    ...
    if ((article.articleInvoiceEntryTaxes?.length || 0) >= taxes.length) {
      toast.info(tInvoicing('invoice.errors.surpassed_tax_limit'));
      return;
    }
  ...
  return (
    ...
    >
      <div className="space-y-3">
        <div className="space-y-2">
          ...
          <Input
            className="h-10"
            disabled={!edit}
            placeholder={tInvoicing('invoice.editor.product_placeholder')}
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-sm">
            ...
            <Label htmlFor={descriptionToggleId}>
              {tInvoicing('invoice.editor.show_description')}
            </Label>
          </div>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-sm">
            ...
            />
            <Label htmlFor={discountToggleId}>{tInvoicing('invoice.editor.apply_discount')}</Label>
          </div>
        ...
        {!canReadTaxes ? (
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              >
                <SelectTrigger className="h-10">
                  <SelectValue placeholder={tInvoicing('invoice.editor.select_taxes')} />
                </SelectTrigger>
                <SelectContent align="start">
            ...
            {(article.articleInvoiceEntryTaxes?.length || 0) > 0 &&
              (article.articleInvoiceEntryTaxes?.length || 0) < taxes.length && (
                <Button
                  type="button"
                  variant="outline"
                ...
                >
                  {tInvoicing('invoice.editor.select_taxes')}
                </Button>
              )}
          </div>
        ) : (
          <div className="space-y-2 pt-2 text-sm text-zinc-600 dark:text-zinc-300">
            ...
            {(article.articleInvoiceEntryTaxes || []).map((entry, index) => (
      ...
      <div className="space-y-2">
        ...
        <div className="flex h-10 items-center rounded-md border border-zinc-200 bg-zinc-50 px-3 text-sm text-zinc-500 dark:border-zinc-800 dark:bg-zinc-900/30 dark:text-zinc-300">
          {article.subTotal?.toFixed(currency?.digitAfterComma || 3) ||
            (0).toFixed(currency?.digitAfterComma || 3)}{' '}
          {currencySymbol}
        </div>
      </div>
    </div>

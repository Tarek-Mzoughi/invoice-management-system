...
export const InvoiceEmailDialog: React.FC<InvoiceEmailDialogProps> = ({
  ...
  React.useEffect(() => {
    if (invoice && open) {
      ...
      const dateStr = invoice.date
        ? format(parseISO(invoice.date), '6 MMMM yyyy', { locale: fr })
        : '';
      const amount =
        invoice.total?.toLocaleString('fr-FR', { minimumFractionDigits: 3 }) || '0,000';
      const currency = invoice.currency?.symbol || 'DT';
      const clientName = invoice.firm?.name || invoiceLabels.partnerFallback;

      setSubject(`[Tarek Mzoughi] ${invoiceLabels.singular} ${reference}`);
      setRecipient(invoice.interlocutor?.email || 'client@example.com');
  ...
  const handleSend = async () => {
    ...
    setIsSending(true);
    try {
      await api.invoice.sendEmail(invoice.id, {
        to: recipient,

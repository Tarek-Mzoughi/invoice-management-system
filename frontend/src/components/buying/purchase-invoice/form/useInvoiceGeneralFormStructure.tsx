...
export const useInvoiceGeneralFormStructure = ({
  ...
  const firmLabel = tInvoice('attributes.supplier');
  const firmPlaceholder = tInvoice('associate_supplier');
  ...
  const referenceField: Field<TextFieldProps | CustomFieldProps> = {
    id: 'reference',
    label: `${tInvoice('attributes.reference')} (*)`,
    variant: FieldVariant.TEXT,
    placeholder: tInvoice('placeholders.reference'),
    props: {
      value: invoiceManager.reference || '',
      onChange: (value) => invoiceManager.set('reference', value),
      disabled: !edit || loading
    }
  };

...
export const InvoiceDetailsDialog: React.FC<InvoiceDetailsDialogProps> = ({
  ...
  const visibleCreditNotes = (invoice.creditNotes || []).filter(
    (creditNote) => creditNote.status !== CREDIT_NOTE_DRAFT_STATUS
  );
      ...
      <div className="space-y-4">
        ...
        <div className="overflow-hidden rounded-md border border-zinc-200 dark:border-zinc-800">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
              {visibleCreditNotes.map((creditNote) => (
                <tr key={`credit-note-${creditNote.id}`}>
                  ...
                  <td className="px-4 py-3 text-right">
                    <Button
                      ...
                      onClick={() =>
                        creditNote.id &&
                        onLinkedDocumentPreview &&
                        onLinkedDocumentPreview('credit-note', creditNote.id)
                      }
                      disabled={!onLinkedDocumentPreview}
                    >
              ...
              {invoice.quotation && (
                <tr key={`quotation-${invoice.quotation.id}`}>
                  ...
                  <td className="px-4 py-3 text-right">
                    <Button
                      ...
                      onClick={() =>
                        invoice.quotationId &&
                        onLinkedDocumentPreview &&
                        onLinkedDocumentPreview('quotation', invoice.quotationId)
                      }
                      disabled={!onLinkedDocumentPreview}
                    >
                      {tInvoicing('invoice.action_view_document') || 'Voir le Document'}
              ...
              {(invoice.customerOrder?.id || invoice.customerOrderId) && (
                <tr key={`customer-order-${invoice.customerOrder?.id || invoice.customerOrderId}`}>
              ...
              {(invoice.deliveryNote?.id || invoice.deliveryNoteId) && (
                <tr key={`delivery-note-${invoice.deliveryNote?.id || invoice.deliveryNoteId}`}>
  ...
  return (
    <Sheet open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <SheetContent
        hideClose
        className="flex h-full w-full flex-col border-l border-zinc-200 bg-white p-0 shadow-xl dark:border-zinc-800 dark:bg-zinc-950 sm:max-w-[620px]"
      >
          ...
          <div className="flex items-center gap-2">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-64">
                ...
                {invoice.status !== INVOICE_STATUS.Draft && !isTransformed && onCreditNote && (
                  <DropdownMenuItem onClick={onCreditNote} className="gap-2">
                ...
                {isTransformed && (
                  <DropdownMenuItem
                    onClick={() => setActiveTab('linked-documents')}
                    className="gap-2"
                  >
                    <ExternalLink className="h-4 w-4" />
                ...
                {onWhatsApp && (
                  <DropdownMenuItem onClick={onWhatsApp} className="gap-2">
                ...
                {onAttachment && (
                  <DropdownMenuItem onClick={onAttachment} className="gap-2">
                    <Paperclip className="h-4 w-4" />
                    {tInvoicing('invoice.action_attachments')}
                  </DropdownMenuItem>
                )}
                {onDownloadWithAttachments && (
                  <DropdownMenuItem onClick={onDownloadWithAttachments} className="gap-2">
                    <Paperclip className="h-4 w-4" />
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
          <div className="space-y-6">
            <div className="space-y-3">
              ...
              <button
                type="button"
                className="text-left text-2xl font-semibold text-blue-600 hover:text-blue-700"
                onClick={() => invoice.firmId && router.push(`/contacts/firm/${invoice.firmId}`)}
              >

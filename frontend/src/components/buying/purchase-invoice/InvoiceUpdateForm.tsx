...
export const InvoiceUpdateForm = ({
  ...
  React.useEffect(() => {
    const displayNumber = isBuying
      ...
      : invoice?.sequential;
    const isSellingDraftInvoice = false;
    setIntro?.(
      displayNumber ? `${invoiceLabels.singular} N° ${displayNumber}` : invoiceLabels.editTitle,
      ''
    );
    if (displayNumber) {
      setRoutes?.([
  ...
  const isDraft = invoiceManager.status === INVOICE_STATUS.Draft;
    ...
    <div className={cn('flex-1 overflow-auto py-6', className)}>
      ...
      <InvoiceSettingsDialog open={settingsDialogOpen} onOpenChange={setSettingsDialogOpen}>
        <InvoiceControlSection
          ...
          showPayments={false}
          invoicePathPrefix={'/buying/facture-achat'}

...
export const InvoicePortal: React.FC<InvoicePortalProps> = ({
  className,
  firmId,
  interlocutorId,
  scope = 'buying',
  ...
  const [detailsInitialTab, setDetailsInitialTab] = React.useState<'payments' | 'linked-documents'>(
    'payments'
  );
  ...
  const isSellingInvoiceEditable = React.useCallback((invoice: Invoice) => true, [scope]);
  const hasLinkedCreditNotes = React.useCallback(
    (invoice: Invoice) =>
      !!invoice.creditNotes?.some((creditNote) => creditNote.status !== 'creditNote.status.draft'),
    []
  );
  const canTransformToCreditNote = React.useCallback(
    (invoice: Invoice) => invoice.status !== INVOICE_STATUS.Draft && !hasLinkedCreditNotes(invoice),
    [hasLinkedCreditNotes]
  );
  const openInvoiceDetails = React.useCallback(
    (invoice: Invoice, initialTab: 'payments' | 'linked-documents' = 'payments') => {
  ...
  const { mutate: bulkTransformToCreditNote, isPending: isBulkCreditNotePending } = useMutation({
    ...
    onError: (error) => {
      toast.error(
        getErrorMessage('invoicing', error, tInvoicing('transformations.creditNote.failure'))
      );
    }
  });
        ...
        {selectedIds.length > 0 && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="outline"
                className="h-11 rounded-md px-5 border-zinc-200 dark:border-zinc-800"
              >
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              <DropdownMenuItem
                onClick={() => bulkTransformToCreditNote(selectedIds)}
                className="gap-2"
              >
  ...
  const setManagedInvoice = React.useCallback(
    (invoice: Invoice) => {
  ...
  const { mutate: toDeliveryNote, isPending: isDeliveryNotePending } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      router.push(`${rootPath}/bon-reception/${data.id}`);
    },
    onError: (error) => {
      toast.error(
  ...
  const { mutate: toCreditNote, isPending: isCreditNotePending } = useMutation({
    ...
    onSuccess: (data) => {
      toast.success(tInvoicing('transformations.creditNote.success'));
      router.push(`${rootPath}/avoir-fournisseur/${data.id}`);
    },
    onError: (error) => {
      toast.error(
        getErrorMessage('invoicing', error, tInvoicing('transformations.creditNote.failure'))
  ...
  const { mutate: toReturnNote, isPending: isReturnNotePending } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      router.push(`${rootPath}/retour-fournisseur/${data.id}`);
    },
    onError: (error) => {
      toast.error(
  ...
  return (
    <>
      ...
      <InvoiceActionDialog
        ...
        callback={() => invoiceManager?.id && toDeliveryNote(invoiceManager.id)}
        isCallbackPending={isDeliveryNotePending}
        onClose={() => setDeliveryNoteDialog(false)}
      />
      <InvoiceActionDialog
        id={invoiceManager?.id}
        action={`${tCommon('commands.transform_to')} ${creditNoteLabels.singular}`}
        sequential={invoiceManager?.sequential || ''}
        open={creditNoteDialog}
        callback={() => invoiceManager?.id && toCreditNote(invoiceManager.id)}
        isCallbackPending={isCreditNotePending}
        onClose={() => setCreditNoteDialog(false)}
      />
      <InvoiceActionDialog
        id={invoiceManager?.id}
        ...
        callback={() => invoiceManager?.id && toReturnNote(invoiceManager.id)}
      ...
      <InvoiceDetailsDialog
        ...
        onPreview={() => {
          ...
          loadPreview(selectedInvoice.id!);
        }}
        onDownload={() => {
          if (!selectedInvoice || !setManagedInvoice(selectedInvoice)) return;
        ...
        onLinkedDocumentPreview={(type, id) => {
          switch (type) {
            case 'credit-note': {
              const creditNote = selectedInvoice?.creditNotes?.find(
            ...
            case 'quotation':
              ...
              break;
            case 'customer-order':
              router.push(`${rootPath}/commande-fournisseur/${id}`);
              break;
            case 'delivery-note':
              router.push(`${rootPath}/bon-reception/${id}`);
              break;
            case 'invoice':
              router.push(`${rootPath}/facture-achat/${id}`);
              break;
            default:
      ...
      <DocumentPreviewDialog
        ...
        filename={`${invoiceManager.sequential || invoiceManager.reference || `invoice-${invoiceManager.id || 'preview'}`}.pdf`}
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-zinc-200 bg-zinc-200 text-left text-sm text-zinc-700 dark:border-zinc-800 dark:bg-zinc-800 dark:text-zinc-300">
                  ...
                  <th className="px-4 py-3 font-medium">{invoiceLabels.referenceFieldLabel}</th>
                  <th className="px-4 py-3 font-medium">{partnerLabel}</th>
              ...
              <tbody>
                {!isPending && invoices.length > 0 ? (
                  invoices.map((invoice) => {
                    ...
                    return (
                      <tr
                        key={invoiceId || invoice.sequential}
                        className="border-b border-zinc-200/70 text-sm text-zinc-700 transition hover:bg-zinc-200/40 dark:border-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-900/80"
                      >
                        ...
                        <td className="px-4 py-3 align-middle">
                          <div className="flex flex-col">
                            <button
                              ...
                              onClick={() => router.push(`${detailPathPrefix}/${invoice.id}`)}
                            >
                          ...
                          <div className="flex items-center justify-end gap-1">
                            {true && (
                              <>
                                <Button
                                  ...
                                  onClick={() => {
                                    if (!setManagedInvoice(invoice)) return;
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-64 p-1">
                                ...
                                {canTransformToCreditNote(invoice) && (
                                  <DropdownMenuItem
                                    className="gap-2 px-3 py-2 cursor-pointer"
                                    onClick={() => {
                                      if (!setManagedInvoice(invoice)) return;
                                ...
                                {hasLinkedCreditNotes(invoice) && (
                                  <DropdownMenuItem
                                    className="gap-2 px-3 py-2 cursor-pointer"
                                    onClick={() => openInvoiceDetails(invoice, 'linked-documents')}
                                  >
                                ...
                                {invoice.uploads && invoice.uploads.length > 0 && (
                                  <DropdownMenuItem
                                    onClick={() => handleDownloadWithAttachments(invoice)}
                                    className="gap-2 px-3 py-2 cursor-pointer"
                                  >

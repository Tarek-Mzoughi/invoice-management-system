...
interface ReturnNoteSettingsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  children: React.ReactNode;
}

export const ReturnNoteSettingsDialog = ({
  open,
  onOpenChange,
  children
}: ReturnNoteSettingsDialogProps) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[calc(100vw-2rem)] max-w-[760px] gap-0 p-0">
        <DialogHeader className="border-b border-zinc-200 px-6 py-5 pr-12 dark:border-zinc-800">
          <DialogTitle>{tInvoicing('returnNote.settings_dialog.title')}</DialogTitle>
          <DialogDescription>
            {tInvoicing('returnNote.settings_dialog.description')}
          </DialogDescription>
        </DialogHeader>

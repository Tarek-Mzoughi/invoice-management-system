import { ArticleDeliveryNoteEntry, Tax } from '@/types';
import { DISCOUNT_TYPE } from '@/types/enums/discount-types';
import { v4 as uuidv4 } from 'uuid';
import { create } from 'zustand';

type DeliveryNotePseudoItem = {
  id: string;
  article: ArticleDeliveryNoteEntry & { total?: number };
};

export type DeliveryNoteArticleManager = {
  articles: DeliveryNotePseudoItem[];
  taxSummary: { tax: Tax; amount: number }[];
  add: (article?: ArticleDeliveryNoteEntry) => void;
  update: (id: string, article: ArticleDeliveryNoteEntry) => void;
  delete: (id: string) => void;
  setArticles: (articles: ArticleDeliveryNoteEntry[]) => void;
  reset: () => void;
  getArticles: () => (ArticleDeliveryNoteEntry & { total: number })[];
  //feature
  removeArticleDescription: () => void;
};

const calculateForDeliveryNote = (article: ArticleDeliveryNoteEntry) => {
  ...
  if (article?.articleDeliveryNoteEntryTaxes) {
    for (const entry of article?.articleDeliveryNoteEntryTaxes) {
      if (entry?.tax?.isRate) {
...
export const useDeliveryNoteArticleManager = create<DeliveryNoteArticleManager>()((set, get) => ({
  articles: [],
  taxSummary: [],

  add: (article: ArticleDeliveryNoteEntry = {}) => {
    const { subTotal, total } = calculateForDeliveryNote(article);
  ...
  update: (id: string, article: ArticleDeliveryNoteEntry) => {
    const { subTotal, total } = calculateForDeliveryNote(article);
  ...
  setArticles: (articles: ArticleDeliveryNoteEntry[]) => {
    set({
  ...
  getArticles: () => {
    return get().articles.map((item) => {
      const { subTotal, total } = calculateForDeliveryNote(item.article);

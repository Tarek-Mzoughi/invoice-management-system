...
export const CustomerOrderPortal: React.FC<CustomerOrderMainProps> = ({
  className,
  scope = 'buying',
  rootPath = '/buying',
  ...
  linkedDeliveryNotePathPrefix = '/buying/bon-reception'
}) => {
  ...
  const partnerLabel = tInvoicing('customerOrder.attributes.supplier');
  const partnerPlaceholder = tCommon('filters.select_supplier');
  const allPartnersLabel = tCommon('filters.all_suppliers');
  ...
  const [selectedCustomerOrder, setSelectedCustomerOrder] = React.useState<CustomerOrder | null>(
    null
  );
  const { value: debouncedSearchTerm, loading: searching } = useDebounce<string>(searchTerm, 400);
  const { value: debouncedFilters, loading: filtering } = useDebounce<CustomerOrderFilters>(
    filters,
    400
  );
  React.useEffect(() => {
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              ...
              className={cn(
                ...
                'h-11 rounded-sm px-4 bg-white dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800 text-sm font-medium text-zinc-900 dark:text-zinc-100 hover:bg-zinc-50 dark:hover:bg-zinc-900 shadow-sm'
              )}
            >
            ...
            <DropdownMenuContent align="end" className="w-64">
              ...
              <DropdownMenuItem
                onClick={() => bulkTransformToDeliveryNote(selectedIds)}
                disabled={selectedIds.length === 0}
                className="cursor-pointer"
              >
                <ArrowUpRight className="mr-2 h-4 w-4" />
              ...
              <DropdownMenuItem
                onClick={() => bulkRemoveCustomerOrder(selectedIds)}
                disabled={selectedIds.length === 0}
  ...
  const {
    ...
    queryFn: () =>
      api.customerOrder.findPaginated(page, size, 'DESC', 'createdAt', {
  ...
  const { data: firms = [], isPending: isFetchFirmsPending } = useQuery({
    ...
    queryFn: () => api.firm.findChoices([], 'suppliers'),
    staleTime: 60000
  });
  ...
  React.useEffect(() => {
    setSelectedIds((current) =>
      current.filter((id) => customerOrders.some((customerOrder) => customerOrder.id === id))
    );
  }, [customerOrders]);
  React.useEffect(() => {
  ...
  const hasActiveFilters =
    ...
    filters.minTotal !== '' ||
    filters.maxTotal !== '';
  const handleFilterChange = <K extends keyof CustomerOrderFilters>(
    key: K,
    value: CustomerOrderFilters[K]
  ) => {
    setPage(1);
    setFilters((current) => ({
      ...current,
  ...
  const setManagedCustomerOrder = React.useCallback(
    (customerOrder: CustomerOrder) => {
      if (!customerOrder.id) return false;
  ...
  const closeCustomerOrderDetailsThen = React.useCallback(
  ...
  const { mutate: loadDocumentPreview, isPending: isLinkedPreviewPending } = useMutation({
    mutationFn: (data: { type: string; id: number }) => {
      switch (data.type) {
        case 'invoice':
  ...
  const openLinkedDocumentPreview = React.useCallback(
    (type: string, id: number) => {
      const linkedSequential =
        type === 'invoice'
          ? invoiceLabels.displayNumber(
              selectedCustomerOrder?.invoices?.find((invoice) => invoice.id === id)
            )
          : type === 'delivery-note'
            ? deliveryNoteLabels.displayNumber(
                selectedCustomerOrder?.deliveryNotes?.find((deliveryNote) => deliveryNote.id === id)
              )
  ...
  const { mutate: updateStatus, isPending: isUpdateStatusPending } = useMutation({
    mutationFn: (data: { id: number; status: CUSTOMER_ORDER_STATUS }) =>
    ...
    onError: (error) => {
      toast.error(
        getErrorMessage('invoicing', error, tInvoicing('customerOrder.action_update_failure'))
      );
    }
  });
  ...
  const { mutate: toDeliveryNote, isPending: isDeliveryNotePending } = useMutation({
    ...
    onError: (error) => {
      toast.error(
        getErrorMessage('invoicing', error, tInvoicing('transformations.deliveryNote.failure'))
      );
    }
  });
  // --- Bulk Actions ---
  ...
  const { mutate: bulkTransformToInvoice, isPending: isBulkInvoicePending } = useMutation({
    ...
    onError: (error) => {
      toast.error(
        getErrorMessage('invoicing', error, tInvoicing('customerOrder.action_invoice_failure'))
      );
    }
  });
  const { mutate: bulkTransformToDeliveryNote, isPending: isBulkDNPending } = useMutation({
    ...
    onError: (error) => {
      toast.error(
        getErrorMessage('invoicing', error, tInvoicing('deliveryNote.action_create_failure'))
      );
    }
  });
  ...
  return (
    <>
      ...
      <DocumentPreviewDialog
        open={previewDialog}
        loading={isPreviewPending || isLinkedPreviewPending}
      ...
      <CustomerOrderDetailsDialog
        ...
        onPreview={() => {
          ...
          closeCustomerOrderDetailsThen(() => {
            setPreviewFilename(
              `${customerOrderLabels.displayNumber(selectedCustomerOrder) || `customerOrder-${selectedCustomerOrder.id || 'preview'}`}.pdf`
            );
            setPreviewBlob(null);
            setPreviewDialog(true);
        ...
        onDownload={() => {
          if (!selectedCustomerOrder || !setManagedCustomerOrder(selectedCustomerOrder)) return;
      ...
      />
      <CustomerOrderEmailDialog
        open={emailDialog}
        onClose={() => setEmailDialog(false)}
        customerOrder={selectedCustomerOrder}
        scope={scope}
      />
      <CustomerOrderWhatsAppDialog
        open={whatsAppDialog}
        onClose={() => setWhatsAppDialog(false)}
        customerOrder={selectedCustomerOrder}
        scope={scope}
      />
      <CustomerOrderAttachmentDialog
      ...
      <CustomerOrderActionDialog
        id={customerOrderManager?.id}
        action={`${tCommon('commands.transform_to')} ${deliveryNoteLabels.singular}`}
        sequential={customerOrderManager?.sequential || ''}
        open={deliveryNoteDialog}
        callback={() => customerOrderManager?.id && toDeliveryNote(customerOrderManager.id)}
        isCallbackPending={isDeliveryNotePending}
        scope={scope}
        onClose={() => setDeliveryNoteDialog(false)}
      />
      <div className={cn('flex min-h-0 flex-1 flex-col gap-5 pb-5', className)}>
        ...
        <section className={cn(panelClassName, 'flex min-h-0 flex-1 flex-col')}>
          {selectedIds.length > 0 && (
            <div className="flex shrink-0 items-center justify-between bg-blue-50/50 dark:bg-blue-950/30 px-4 py-2 text-sm border-b border-zinc-200 dark:border-zinc-800">
              ...
              <button
                ...
                className="p-1 rounded-sm text-blue-600 hover:bg-blue-100 dark:text-blue-400 dark:hover:bg-blue-900 transition-colors"
          ...
          <div className="min-h-0 flex-1 overflow-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-zinc-200 bg-zinc-100 text-left text-sm text-zinc-700 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300">
                  ...
                  <th className="px-4 py-3.5 font-medium">
                    {customerOrderLabels.referenceFieldLabel}
                  </th>
              ...
              <tbody>
                {!isPending && customerOrders.length > 0 ? (
                  customerOrders.map((customerOrder) => {
                    ...
                    return (
                      <tr
                        key={customerOrderId || customerOrder.sequential}
                      ...
                      >
                        ...
                        <td className="px-4 py-4 align-middle">
                          <div className="flex flex-col">
                            <button
                              ...
                              onClick={() => guardedNavigation.push(`${detailPathPrefix}/${customerOrder.id}`)}
                            >
                        ...
                        <td className="px-4 py-4 align-middle">
                          {(() => {
                            ...
                            const hasEffectiveDelivery = customerOrder.deliveryNotes?.some(
                              (deliveryNote) =>
                            ...
                            return (
                              <div className="flex items-center gap-2">
                                ...
                                {hasEffectiveDelivery && (
                                  <span className="inline-flex rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700 dark:border-blue-900 dark:bg-blue-950/40 dark:text-blue-300">
                                    {tInvoicing('customerOrder.status.delivered')}
                                  </span>
                                )}

                                {isEffectivelyValidated && (
                          ...
                          <div className="flex items-center justify-end gap-1">
                            <Button
                              ...
                              onClick={() => {
                                ...
                                setPreviewFilename(
                                  `${customerOrder.sequential || `customerOrder-${customerOrder.id || 'preview'}`}.pdf`
                                );
                                setPreviewBlob(null);
                                setPreviewDialog(true);
                            ...
                            <DropdownMenu>
                              <DropdownMenuTrigger
                                ...
                                className={cn(
                                  ...
                                  'h-8 w-8 rounded-sm text-zinc-500 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-zinc-900'
                                )}
                              >
                              ...
                              <DropdownMenuContent align="end" className="w-64">
                                ...
                                <DropdownMenuItem
                                  onClick={() =>
                                    guardedNavigation.push(`${detailPathPrefix}/${customerOrder.id}`)
                                  }
                                >
                                ...
                                <>
                                  ...
                                  0 ? (
                                    ...
                                    <>
                                      <DropdownMenuItem
                                        onClick={() => {
                                          if (!setManagedCustomerOrder(customerOrder)) return;
                                ...
                                {customerOrder.uploads && customerOrder.uploads.length > 0 && (
                                  <DropdownMenuItem
                                    onClick={() => handleDownloadWithAttachments(customerOrder)}
                                  >

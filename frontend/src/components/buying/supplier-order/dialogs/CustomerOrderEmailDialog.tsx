...
import { api } from '@/api';
interface CustomerOrderEmailDialogProps {
  open: boolean;
  onClose: () => void;
  customerOrder: CustomerOrder | null;
  scope?: 'selling' | 'buying';
}
export const CustomerOrderEmailDialog: React.FC<CustomerOrderEmailDialogProps> = ({
  open,
  onClose,
  customerOrder,
  scope = 'buying'
  ...
  React.useEffect(() => {
    if (customerOrder && open) {
      const reference = documentLabels.displayNumber(customerOrder);
      const dateStr = customerOrder.date
        ? format(parseISO(customerOrder.date), '6 MMMM yyyy', { locale: fr })
        : '';
      const amount =
        customerOrder.total?.toLocaleString('fr-FR', { minimumFractionDigits: 3 }) || '0,000';
      const currency = customerOrder.currency?.symbol || 'DT';
      const supplierName = customerOrder.firm?.name || documentLabels.partnerFallback;
      setSubject(`[Tarek Mzoughi] ${documentLabels.singular} ${reference}`);
      setRecipient(customerOrder.interlocutor?.email || 'fournisseur@example.com');
      setMessage(
        `Cher(e) ${supplierName},\n\n` +
  ...
  }, [customerOrder, documentLabels, open]);
  const handleSend = async () => {
    ...
    setIsSending(true);
    try {
      await api.customerOrder.sendEmail(customerOrder.id, {
        to: recipient.trim(),

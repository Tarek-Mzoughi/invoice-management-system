...
import { CUSTOMER_ORDER_STATUS, CustomerOrder, CustomerOrderUploadedFile } from '@/types';
...
interface CustomerOrderAttachmentDialogProps {
  open: boolean;
  onClose: () => void;
  customerOrder: CustomerOrder | null;
  scope?: 'selling' | 'buying';
}
export const CustomerOrderAttachmentDialog: React.FC<CustomerOrderAttachmentDialogProps> = ({
  open,
  onClose,
  customerOrder,
  scope = 'buying'
  ...
  const [initialUploads, setInitialUploads] = React.useState<CustomerOrderUploadedFile[]>([]);
  const [previewFile, setPreviewFile] = React.useState<File | null>(null);
  const [previewUrl, setPreviewUrl] = React.useState<string>('');
  const { data: fullCustomerOrder, isLoading } = useQuery({
    queryKey: ['customerOrder-details', customerOrder?.id],
    queryFn: () => api.customerOrder.findOne(customerOrder!.id!),
    enabled: open && !!customerOrder?.id
  });
  React.useEffect(() => {
    if (fullCustomerOrder && fullCustomerOrder.files) {
      setInitialUploads(fullCustomerOrder.files);
      setFiles(fullCustomerOrder.files.map((u) => u.file).filter((f): f is File => !!f));
    } else {
      setFiles([]);
      setInitialUploads([]);
    }
  }, [fullCustomerOrder, open]);
  ...
  const { mutate: updateCustomerOrderFiles, isPending: isUpdating } = useMutation({
    mutationFn: async (updatedFiles: File[]) => {
      if (!fullCustomerOrder) return;
      ...
      const articlesDto = fullCustomerOrder.articleCustomerOrderEntries?.map((article: any) => ({
        article: {
        ...
        taxes:
          article?.articleCustomerOrderEntryTaxes?.map(
            (entry: any) => entry?.tax?.id || entry?.taxId
          ) || []
      }));
      const payload: any = {
        ...fullCustomerOrder,
        firmId: fullCustomerOrder.firmId || fullCustomerOrder.firm?.id,
        interlocutorId: fullCustomerOrder.interlocutorId || fullCustomerOrder.interlocutor?.id,
        currencyId: fullCustomerOrder.currencyId || fullCustomerOrder.currency?.id,
        bankAccountId: fullCustomerOrder.bankAccountId || fullCustomerOrder.bankAccount?.id,
      ...
      return api.customerOrder.update(payload, newFiles);
    },
    onSuccess: () => {
  ...
  const handleSave = () => {
    updateCustomerOrderFiles(files);
  };
  ...
  // Safe fallback to customerOrder if fullCustomerOrder is loading
  const displayQuot = fullCustomerOrder || customerOrder;
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100 dark:border-zinc-900">
            ...
            <SheetDescription className="sr-only">
              Gérer les pièces jointes de la {documentLabels.document.toLowerCase()}
            </SheetDescription>
              ...
              <div className="flex items-start justify-between w-full">
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-semibold">
                    {documentLabels.displayNumber(displayQuot)}
                  </p>
                </div>
                <Badge
                  variant="outline"
                ...
                <div className="space-y-1">
                  <p className="text-xs text-zinc-500 font-medium">
                    {documentLabels.isBuying
                      ? tInvoicing('customerOrder.attributes.supplier')
                      : tInvoicing('customerOrder.attributes.customer')}
                  </p>
                  <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">
                    {displayQuot?.firm?.name || documentLabels.partnerFallback}
                  </p>
                </div>
                <div className="space-y-1">

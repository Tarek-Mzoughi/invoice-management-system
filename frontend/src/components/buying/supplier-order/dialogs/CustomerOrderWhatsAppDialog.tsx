...
interface CustomerOrderWhatsAppDialogProps {
  open: boolean;
  onClose: () => void;
  customerOrder: CustomerOrder | null;
  scope?: 'selling' | 'buying';
}
export const CustomerOrderWhatsAppDialog: React.FC<CustomerOrderWhatsAppDialogProps> = ({
  open,
  onClose,
  customerOrder,
  scope = 'buying'
  ...
  React.useEffect(() => {
    if (customerOrder && open) {
      setPhone(customerOrder.interlocutor?.phone || '');
    }
  }, [customerOrder, open]);
  const handleSend = async () => {
    ...
    setIsSending(true);
    try {
      const blob = await api.customerOrder.preview(customerOrder.id, 'template1');

...
import { ArticleCustomerOrderEntry, Currency, Tax } from '@/types';
...
interface CustomerOrderTaxEntriesProps {
  className?: string;
...
export const CustomerOrderTaxEntries: React.FC<CustomerOrderTaxEntriesProps> = ({
  className,
  article,
  taxes,
  selectedTaxIds,
  ...
  return (
    <div className={cn('flex flex-col', className)}>
      {(!article?.articleCustomerOrderEntryTaxes ||
        article?.articleCustomerOrderEntryTaxes?.length == 0) && (
        <p className="font-thin text-sm m-1">{tInvoicing('article.no_applied_tax')}</p>
      )}
      {article.articleCustomerOrderEntryTaxes?.map((appliedTax, i) => (
        <div className="flex items-center justify-between gap-2" key={i}>
          {appliedTax?.tax ? (
            ...
            <Select
              ...
              value={appliedTax?.tax?.id?.toString() || ''}
            >
              <SelectTrigger>
              ...
              <SelectContent align="center">
                {rateTaxes.length != 0 && (
                  <SelectGroup>
                    <SelectLabel>{tInvoicing('article.attributes.percentages')}</SelectLabel>
                    {rateTaxes.map((tax) => (
                ...
                {fixedAmountTaxes.length != 0 && (
                  <SelectGroup>
                    <SelectLabel>{tInvoicing('article.attributes.fixed_price')}</SelectLabel>

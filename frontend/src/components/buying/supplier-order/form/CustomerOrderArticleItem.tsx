...
import { ArticleCustomerOrderEntry, Currency, CustomerOrderTaxEntry, Tax } from '@/types';
import {
  Select,
  SelectTrigger,
  SelectContent,
...
interface CustomerOrderArticleItemProps {
  className?: string;
  article: ArticleCustomerOrderEntry;
  onChange: (item: ArticleCustomerOrderEntry) => void;
...
export const CustomerOrderArticleItem: React.FC<CustomerOrderArticleItemProps> = ({
  className,
  article,
  onChange,
  taxes,
  ...
  const selectedTaxIds = React.useMemo(
    () => (canReadTaxes ? article.articleCustomerOrderEntryTaxes?.map((taxEntry) => taxEntry.tax?.id) || [] : []),
    [article.articleCustomerOrderEntryTaxes, canReadTaxes]
  );
  const primaryTax = canReadTaxes ? article.articleCustomerOrderEntryTaxes?.[0] : undefined;
  const extraTaxes = React.useMemo(
    () => (canReadTaxes ? article.articleCustomerOrderEntryTaxes?.slice(1) || [] : []),
    [article.articleCustomerOrderEntryTaxes, canReadTaxes]
  );

  const getAvailableTaxes = React.useCallback(
    (index: number) => {
      const currentTaxId = article.articleCustomerOrderEntryTaxes?.[index]?.tax?.id;
      return canReadTaxes ? taxes.filter((tax) => !selectedTaxIds.includes(tax.id) || tax.id === currentTaxId) : [];
    },
    [article.articleCustomerOrderEntryTaxes, canReadTaxes, selectedTaxIds, taxes]
  );

  const updateArticle = React.useCallback(
    (partial: Partial<ArticleCustomerOrderEntry>) => {
      onChange({
        ...article,
        ...partial
      });
  ...
  const handleTaxChange = (value: string, index: number) => {
    ...
    const updatedTaxes = [...(article.articleCustomerOrderEntryTaxes || [])];
    ...
    updateArticle({ articleCustomerOrderEntryTaxes: updatedTaxes });
  };

  const handleTaxDelete = (index: number) => {
    if (!canReadTaxes) return;
    const updatedTaxes = article.articleCustomerOrderEntryTaxes?.filter((_, i) => i !== index);
    updateArticle({ articleCustomerOrderEntryTaxes: updatedTaxes });
  };

  const handleAddTax = () => {
    if (!canReadTaxes) return;
    if ((article.articleCustomerOrderEntryTaxes?.length || 0) >= taxes.length) {
      toast.info(tInvoicing('customerOrder.errors.surpassed_tax_limit'));
      return;
    }

    updateArticle({
      articleCustomerOrderEntryTaxes: [
        ...(article.articleCustomerOrderEntryTaxes || []),
        {} as CustomerOrderTaxEntry
      ]
    });
  };
      ...
      <div className="space-y-3">
        <div className="space-y-2">
          ...
          <Input
            className="h-10"
            disabled={!edit}
            placeholder={tInvoicing('customerOrder.editor.product_placeholder')}
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-sm">
            ...
            <Label htmlFor={descriptionToggleId}>
              {tInvoicing('customerOrder.editor.show_description')}
            </Label>
          </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-sm">
            ...
            <Label htmlFor={discountToggleId}>
              {tInvoicing('customerOrder.editor.apply_discount')}
            </Label>
          </div>
        ...
        {!canReadTaxes ? (
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              >
                <SelectTrigger className="h-10">
                  <SelectValue placeholder={tInvoicing('customerOrder.editor.select_taxes')} />
                </SelectTrigger>
                <SelectContent align="start">
            ...
            {(article.articleCustomerOrderEntryTaxes?.length || 0) > 0 &&
              (article.articleCustomerOrderEntryTaxes?.length || 0) < taxes.length && (
                <Button
                  type="button"
                  variant="outline"
                ...
                >
                  {tInvoicing('customerOrder.editor.select_taxes')}
                </Button>
              )}
          </div>
        ) : (

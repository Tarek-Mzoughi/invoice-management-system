...
export const CreditNoteDetailsDialog: React.FC<CreditNoteDetailsDialogProps> = ({
  open,
  onClose,
  creditNote,
  ...
  return (
    <Sheet open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <SheetContent
        hideClose
        className="flex h-full flex-col border-l border-zinc-200 bg-white p-0 shadow-xl dark:border-zinc-800 dark:bg-zinc-950 sm:max-w-[680px]"
      >
          ...
          <div className="flex items-center gap-2">
            <DropdownMenu modal={false} open={isActionsOpen} onOpenChange={setIsActionsOpen}>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="outline"
                  className="h-9 gap-2 border-zinc-200 px-3 hover:bg-zinc-50 dark:border-zinc-800 dark:hover:bg-zinc-900"
                >
            ...
            <Button
              variant="outline"
              className="h-9 border-zinc-200 hover:bg-zinc-50 dark:border-zinc-800 dark:hover:bg-zinc-900"
              onClick={onClose}
            >
        ...
        <div className="flex-1 space-y-8 overflow-y-auto px-6 py-8">
          <div className="space-y-4">
            ...
            <div className="pt-2">
              <button
                type="button"
                onClick={() =>
                  creditNote.firmId && router.push(`/contacts/firm/${creditNote.firmId}`)
                }
                className="text-left font-semibold text-blue-600 transition-colors hover:text-blue-700 hover:underline"
              >
                {partnerName}
              </button>
          ...
          <div className="flex flex-wrap gap-3">
            <Button
              variant="outline"
              className="h-10 gap-2 border-zinc-200 shadow-sm hover:bg-zinc-50 dark:border-zinc-800 dark:hover:bg-zinc-900"
              onClick={onDownload}
              disabled={!onDownload}
            >
            ...
            <div className="overflow-hidden rounded-md border border-zinc-100 dark:border-zinc-900">
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
                  {linkedDocuments.map(({ key, type, label, document }) => (
                    <tr
                      key={key}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right">
                        <Button
                          variant="ghost"
                          size="sm"
                          className="font-semibold text-blue-600 hover:bg-blue-50 hover:text-blue-700 dark:text-blue-400 dark:hover:bg-blue-900/20"
                          onClick={() =>
                            document.id &&
                            onLinkedDocumentOpen &&
                            onLinkedDocumentOpen(type, document.id)
                          }
                          disabled={!document.id || !onLinkedDocumentOpen}
                        >
                  ...
                  {!linkedDocuments.length && (
                    <tr>
                      <td
                        colSpan={5}
                        className="px-4 py-8 text-center italic text-zinc-400 dark:text-zinc-600"
                      >

...
export const CreditNoteBulkDeleteDialog: React.FC<CreditNoteBulkDeleteDialogProps> = ({
  className,
  count,
  open,
  onDelete,
  ...
  const header = (
    <Label className="leading-5">
      <Trans
        i18nKey="creditNote.bulk_remove_confirmation"
        t={tInvoicing}
        values={{ count }}
  ...
  if (isDesktop) {
    return (
      <Dialog open={open} onOpenChange={(nextOpen) => !nextOpen && onClose()}>
        <DialogContent className={cn('w-full p-8 sm:max-w-125', className)}>
          <DialogHeader>

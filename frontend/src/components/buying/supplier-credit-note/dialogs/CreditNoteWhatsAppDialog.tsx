...
interface CreditNoteWhatsAppDialogProps {
  open: boolean;
  onClose: () => void;
...
export const CreditNoteWhatsAppDialog: React.FC<CreditNoteWhatsAppDialogProps> = ({
  open,
  onClose,
  creditNote,
  scope
  ...
  React.useEffect(() => {
    if (creditNote && open) {
      setPhone(creditNote.interlocutor?.phone || '');
    }
  }, [creditNote, open]);
  ...
  const handleSend = async () => {
    ...
    setIsSending(true);
    try {
      const blob = await api.creditNote.preview(creditNote.id, 'template1');
      ...
      <DialogContent className="w-full overflow-hidden rounded-xl border border-zinc-200 bg-white p-0 shadow-2xl dark:border-zinc-800 dark:bg-zinc-950 sm:max-w-[600px]">
        ...
        <div className="space-y-6 px-6 py-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="credit-note-whatsapp-phone">Numéro de téléphone</Label>
            <PhoneInput
              id="credit-note-whatsapp-phone"
              value={phone as any}

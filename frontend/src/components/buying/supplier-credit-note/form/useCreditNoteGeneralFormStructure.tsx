...
export const useCreditNoteGeneralFormStructure = ({
  ...
  const documentLabels = useScopedDocumentLabels('creditNote', 'buying');
  const firmLabel = tInvoicing('creditNote.attributes.supplier');
  const firmPlaceholder = tInvoicing('creditNote.associate_supplier');
  ...
  const referenceField: Field<TextFieldProps | CustomFieldProps> = {
    ...
    props: {
      value: creditNoteManager.reference || '',
      onChange: (value) => creditNoteManager.set('reference', value),
      disabled: !edit || loading
    }
  };

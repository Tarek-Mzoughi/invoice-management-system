import { ArticleCreditNoteEntry, Tax } from '@/types';
import { DISCOUNT_TYPE } from '@/types/enums/discount-types';
import { v4 as uuidv4 } from 'uuid';
import { create } from 'zustand';

type CreditNotePseudoItem = { id: string; article: ArticleCreditNoteEntry & { total?: number } };

export type CreditNoteArticleManager = {
  articles: CreditNotePseudoItem[];
  taxSummary: { tax: Tax; amount: number }[];
  add: (article?: ArticleCreditNoteEntry) => void;
  update: (id: string, article: ArticleCreditNoteEntry) => void;
  delete: (id: string) => void;
  setArticles: (articles: ArticleCreditNoteEntry[]) => void;
  reset: () => void;
  getArticles: () => (ArticleCreditNoteEntry & { total: number })[];
  //feature
  removeArticleDescription: () => void;
};

const calculateForCreditNote = (article: ArticleCreditNoteEntry) => {
  ...
  if (article?.articleCreditNoteEntryTaxes) {
    for (const entry of article?.articleCreditNoteEntryTaxes) {
      if (entry?.tax?.isRate) {
...
export const useCreditNoteArticleManager = create<CreditNoteArticleManager>()((set, get) => ({
  articles: [],
  taxSummary: [],

  add: (article: ArticleCreditNoteEntry = {}) => {
    const { subTotal, total } = calculateForCreditNote(article);
  ...
  update: (id: string, article: ArticleCreditNoteEntry) => {
    const { subTotal, total } = calculateForCreditNote(article);
  ...
  setArticles: (articles: ArticleCreditNoteEntry[]) => {
    set({
  ...
  getArticles: () => {
    return get().articles.map((item) => {
      const { subTotal, total } = calculateForCreditNote(item.article);

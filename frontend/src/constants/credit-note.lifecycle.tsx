import { CREDIT_NOTE_STATUS } from '@/types';
import { Archive, Copy, FilePlus, Printer, Save, Send, Trash, X } from 'lucide-react';

export interface CreditNoteLifecycle {
  label: string;
  variant: 'default' | 'outline';
  icon: React.ReactNode;
  when: { set: (CREDIT_NOTE_STATUS | undefined)[]; membership: 'IN' | 'OUT' };
}

export const CREDIT_NOTE_LIFECYCLE_ACTIONS: Record<string, CreditNoteLifecycle> = {
  save: {
    label: 'commands.save',
    variant: 'default',
  ...
  validated: {
    ...
    when: {
      membership: 'IN',
      set: [undefined, CREDIT_NOTE_STATUS.Draft]
    }
  },
  sent: {
    label: 'commands.send',
